...
for i in ['01','02','03','04','05','06','07','08','09','10','11','12']:
    mypath='/Users/santiago/Documents/IASI/NH3/'+i+'/'
    ...
    for file_name in onlyfiles[1:]:
        if file_name[0:11]=='IASI_METOPA':
            ...
            nh3_iasi=iasi.variables['nh3_total_column'][:]
            ...
            # dimensions
            npixel = latitude_iasi[np.logical_not(nh3_iasi.mask)].size
            if npixel<10:
                continue
            #Creating netcdf4
            ...
            NH3 = root_grp.createVariable('NH3', 'f4', ('pixel', 'retr',),fill_value=9.969209968386869e+36)
            ...
            NH3.long_name= 'NH3 total column'
            NH3.units= 'mol m-2'
            NH3.standard_name= 'nh3'
            NH3.multiplication_factor_to_convert_to_molecules_percm2=6.02214076e+19
            ...
            retr[:]=1
            longitude[:]=longitude_iasi[np.logical_not(nh3_iasi.mask)]
            longitude_bounds_aux=np.zeros((npixel,4))
            longitude_bounds_aux[:,0]=longitude_iasi[np.logical_not(nh3_iasi.mask)] -resolution/2
            longitude_bounds_aux[:,1]=longitude_iasi[np.logical_not(nh3_iasi.mask)] +resolution/2
            longitude_bounds_aux[:,2]=longitude_iasi[np.logical_not(nh3_iasi.mask)] +resolution/2
            longitude_bounds_aux[:,3]=longitude_iasi[np.logical_not(nh3_iasi.mask)] -resolution/2
            longitude_bounds[:]=longitude_bounds_aux
            latitude[:]=latitude_iasi[np.logical_not(nh3_iasi.mask)]
            latitude_bounds_aux=np.zeros((npixel,4))
            latitude_bounds_aux[:,0]=latitude_iasi[np.logical_not(nh3_iasi.mask)] -resolution/2
            latitude_bounds_aux[:,1]=latitude_iasi[np.logical_not(nh3_iasi.mask)] -resolution/2
            latitude_bounds_aux[:,2]=latitude_iasi[np.logical_not(nh3_iasi.mask)] +resolution/2
            latitude_bounds_aux[:,3]=latitude_iasi[np.logical_not(nh3_iasi.mask)] +resolution/2
            latitude_bounds[:]=latitude_bounds_aux
            Kernel[:,:]=1
            NH3[:]=nh3_iasi[np.logical_not(nh3_iasi.mask)]
...
pd.DataFrame.from_dict(list_CSO).to_csv('/Users/santiago/Documents/IASI/NH3/listing_IASI.csv',sep=';',index=False)