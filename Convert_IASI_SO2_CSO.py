...
for i in ['05']:
    mypath='/Users/santiago/Documents/IASI/SO2/'
    ...
    for file_name in onlyfiles[1:]:
        if file_name[0:11]=='IASI_METOPA':
            ...
            # dimensions
            npixel = latitude_iasi[so2_iasi>0].size
            if npixel<10:
                continue
            #Creating netcdf4
            ...
            retr[:]=1
            longitude[:]=longitude_iasi[so2_iasi>0]
            longitude_bounds_aux=np.zeros((npixel,4))
            longitude_bounds_aux[:,0]=longitude_iasi[so2_iasi>0] -resolution/2
            longitude_bounds_aux[:,1]=longitude_iasi[so2_iasi>0] +resolution/2
            longitude_bounds_aux[:,2]=longitude_iasi[so2_iasi>0] +resolution/2
            longitude_bounds_aux[:,3]=longitude_iasi[so2_iasi>0] -resolution/2
            longitude_bounds[:]=longitude_bounds_aux
            latitude[:]=latitude_iasi[so2_iasi>0]
            latitude_bounds_aux=np.zeros((npixel,4))
            latitude_bounds_aux[:,0]=latitude_iasi[so2_iasi>0] -resolution/2
            latitude_bounds_aux[:,1]=latitude_iasi[so2_iasi>0] -resolution/2
            latitude_bounds_aux[:,2]=latitude_iasi[so2_iasi>0] +resolution/2
            latitude_bounds_aux[:,3]=latitude_iasi[so2_iasi>0] +resolution/2
            latitude_bounds[:]=latitude_bounds_aux
            Kernel[:,:]=1
            SO2[:]=so2_iasi[so2_iasi>0]
