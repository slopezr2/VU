...
module C3PO_Grid_Ugg
  ...
  contains
    ...
    procedure   ::  OverDomain         => Grid_Ugg_OverDomain

    procedure   ::                        Grid_Ugg_AreaOper_2d
    procedure   ::                        Grid_Ugg_AreaOper_3d
    procedure   ::                        Grid_Ugg_AreaOper_4d
    procedure   ::                        Grid_Ugg_AreaOper_5d
    generic     ::  AreaOper           => Grid_Ugg_AreaOper_2d, &
                                          Grid_Ugg_AreaOper_3d, &
                                          Grid_Ugg_AreaOper_4d, &
                                          Grid_Ugg_AreaOper_5d
...
contains
    ...
    select case (trim(self%type) )
      ...
      !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if ( status /= 0 ) then
          ...
          IF_NOT_OK_RETURN(status=1)
          !~ annote:
          call pf%Add( 'ax.legend()', status, comment='# annnote:' )
          IF_NOT_OK_RETURN(status=1)
          !~ write:
          call pf%Done( status )
          IF_NOT_OK_RETURN(status=1)
  ...
  ! Return indomain=.true. if cell (west,east,south,norht) is completely in domain.  
  ...
  ! Return indomain=.true. if cell (west,east,south,norht) is at least partly over domain.
  
  subroutine Grid_Ugg_OverDomain( self, west,east,south,north, overdomain, status )
  
    ! in/out
    class(T_Grid_Ugg), intent(in)  ::  self
    real, intent(in)               ::  west, east
    real, intent(in)               ::  south, north
    logical, intent(out)           ::  overdomain
    integer, intent(out)           ::  status
    
    ! --- const --------------------------------------

    character(len=*), parameter  ::  rname = mname//'/Grid_Ugg_OverDomain'
    ...
    select case ( trim(self%type) )
      case ( 'cartesian', 'cartesian-regular' )
        overdomain = ( westX <= self%longitude_bnds_1d(2,self%nlon) .and. (eastX >= self%longitude_bnds_1d(1,1)) .and. &
                       south <= self%latitude_bnds_1d (2,self%nlat) .and. (north >= self%latitude_bnds_1d (1,1)) )
      case default
        write (gol,'("Function GetLonIndex not defined for grid type: ",a)') trim(self%type) ; call goErr
        TRACEBACK; status=1; return
  ...
  end subroutine Grid_Ugg_OverDomain
  ...
  subroutine Grid_Ugg_AreaOper_3d( self, ll, oper, unit, status )
    ...
    ! loop over higher dims:
    do k = 1, size(ll,3)
      ! apply 2d operator:
      call self%AreaOper( ll(:,:,k), oper, unit, status )
      IF_NOT_OK_RETURN(status=1)
    end do  ! k
  ...
  subroutine Grid_Ugg_AreaOper_4d( self, ll, oper, unit, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/llgrid_AreaOper_4d'
    ...
    do l = 1, size(ll,4)
      do k = 1, size(ll,3)
        ! apply 2d operator:
        call self%AreaOper( ll(:,:,k,l), oper, unit, status )
        IF_NOT_OK_RETURN(status=1)
      end do  ! k
    end do  ! l
  ...
  end subroutine Grid_Ugg_AreaOper_4d
  
  ! *
  
  subroutine Grid_Ugg_AreaOper_5d( self, ll, oper, unit, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/llgrid_AreaOper_5d'
    ...
    do m = 1, size(ll,5)
      do l = 1, size(ll,4)
        do k = 1, size(ll,3)
          ! apply 2d operator:
          call self%AreaOper( ll(:,:,k,l,m), oper, unit, status )
          IF_NOT_OK_RETURN(status=1)
        end do  ! k
      end do  ! l
    end do  ! m
  ...
  end subroutine Grid_Ugg_AreaOper_5d
