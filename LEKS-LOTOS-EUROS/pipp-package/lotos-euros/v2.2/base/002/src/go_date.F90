...
!
!   Evaluate templates in string, formats recoqnized by python 'datetime.strftime()' method:
!     call goFormat( t, 'output_%Y%m%d.nc', filename, status )
!   will return:
!     filename = 'output_20200723.nc'
!
!
!
! DEFAULTS
...
module GO_Date
  ...
  interface operator(.in.)
    module procedure t_in_tr  ! time in time range
    module procedure tr_in_tr ! time range in time range
  end interface
...
contains
  ...
  logical function t_in_tr( t, tr )
    ...
    if ( tr(2) > tr(1) ) then
      ...
      t_in_tr = t >= tr(1) .and. t <= tr(2) 
    
    else 
      
      t_in_tr = t >= tr(2) .and. t <= tr(1)

    end if
    
  end function t_in_tr
  
 
  ! *** date range in date range ************************
 
  
  logical function tr_in_tr( tr1, tr2 )
    
    ! --- in/out --------------------------------

    type(TDate), intent(in)       ::  tr1(2)
    type(TDate), intent(in)       ::  tr2(2)
    
    ! --- begin ---------------------------------
    
    ! both times from 1 must be in interval 2    
    tr_in_tr = t_in_tr(tr1(1), tr2) .and. t_in_tr(tr1(2), tr2) 
        
  end function tr_in_tr
  ...
  real function date_rTotal( t, unit )
    ...
    select case ( unit )
      ...
      case default
        write (gol,'("unsupported unit `",a,"`")') trim(unit); call goErr
        TRACEBACK; stop
    end select
  ...
  subroutine Get_Surrounding_Interval( t, tref, interpolation, tt, status )
    ...
    select case ( units )
      ...
      case ( 'minutes' )
        ...
        r = rTotal( t - t0, 'min' )
        ! start of interval:
        tt(1) = t0 + IncrDate( min=int(floor(r/step))*step )
        ! end:
        if ( t == tt(1) ) then
          tt(2) = tt(1)
        else
          tt(2) = tt(1) + IncrDate(min=step)
        end if
      ! unknown ...
      case default
  ...
  subroutine goFormat( t, template, value, status )

    use GO_String, only : goReplace
    ...
    character(len=*), parameter  ::  rname = mname//'/goFormat'
    ...
    value = trim(template)
    ! replace:
    call goReplace( value, '%Y', '(i4.4)', t%year , status )
    IF_NOTOK_RETURN(status=1)
    call goReplace( value, '%m', '(i2.2)', t%month, status )
    IF_NOTOK_RETURN(status=1)
    call goReplace( value, '%d', '(i2.2)', t%day  , status )
    IF_NOTOK_RETURN(status=1)
    call goReplace( value, '%H', '(i2.2)', t%hour , status )
    IF_NOTOK_RETURN(status=1)
    call goReplace( value, '%M', '(i2.2)', t%min  , status )
    IF_NOTOK_RETURN(status=1)
    call goReplace( value, '%S', '(i2.2)', t%sec  , status )
    IF_NOTOK_RETURN(status=1)

    ! ok
    status = 0

  end subroutine goFormat
