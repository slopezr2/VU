    ...
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      select case ( trim(fileformat) )
        ...
        ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          call MDF_Inq_DimID( hid, 'nhour', dimid, status )
          if ( status /= 0 ) then
          ...
          ! Length of emission codes
          call MDF_Inq_DimID( hid, 'emis_cat_code_len', dimid, status )
          if ( status /= 0 ) then
            write( gol, '("No dimension category code available, assume length 1") ') ; call goPr
            len_catcode = 1
          else 
            
            call MDF_Inquire_Dimension( hid, dimid, status, length=len_catcode )
            IF_NOTOK_RETURN(status=1)
          end if
          ...
          IF_NOTOK_RETURN(status=1)
          call MDF_Get_StrArr( hid, 'emis_cat', len_catcode, timeprof%ncat, cat_codes_time_prof, status )
          IF_NOTOK_RETURN(status=1)
          ...
          description = 'var_name=hourly_fact;var_name=time_factor'
