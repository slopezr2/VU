...
contains
  ...
  subroutine LE_Landuse_Init( rcF, status )
    ...
    use LE_Landuse_Data   , only : use_corine_landuse
    ...
    use LE_Landuse_Corine , only : LE_Landuse_Corine_Init
    ...
    call ReadRc( rcF, 'le.landuse.corine.enabled', use_corine_landuse, status )
    IF_NOTOK_RETURN(status=1)

    if ( use_corine_landuse ) then

      ! read in corine file
      call LE_Landuse_Corine_Init( rcF, status )
      IF_NOTOK_RETURN(status=1)
    ...
    do iy = 1, ny
      ...
      if ( lu_fracs(ix,iy,ilu_water_sea) > 0 ) then
        ...
        ! part of the water in the cell is inland  
          ...
          lu_fracs(ix,iy,ilu_water_inland) = lu_fracs(ix,iy,ilu_water_inland) + lu_fracs(ix,iy,ilu_water_sea) - waterbodies(ix,iy, 2)
