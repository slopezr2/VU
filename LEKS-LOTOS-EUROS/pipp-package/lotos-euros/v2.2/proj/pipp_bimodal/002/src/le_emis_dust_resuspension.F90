          ...
          if ( n_basecation > 5 ) then
            ...
            do itr = 1, n_basecation
              ...
              select case ( ispec )
                ...
                case(ispec_Na_f2); emis(i,j,1,ispec) = emis(i,j,1,ispec) + emis_f * bcat_frac(i,j,ii_Na)
                case(ispec_Na_f1); emis(i,j,1,ispec) =  emis(i,j,1,ispec) !nothing yet
                case(ispec_Na_c3); emis(i,j,1,ispec) = emis(i,j,1,ispec) + emis_c * bcat_frac(i,j,ii_Na)
                case(ispec_Na_c2); emis(i,j,1,ispec) = emis(i,j,1,ispec) !nothing yet
                case(ispec_Na_c1); emis(i,j,1,ispec) = emis(i,j,1,ispec) !nothing yet
