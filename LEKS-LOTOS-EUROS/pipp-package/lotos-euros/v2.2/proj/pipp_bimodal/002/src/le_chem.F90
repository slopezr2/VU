...
module LE_Chem
  ...
  !Santiago added for SO2 rate perturbations
  public  ::  rk_pertur
  public  ::  SO2_rate_pertur
  ...
  real, allocatable    ::  rk_pertur(:,:)      ! dims (nx,ny)
  logical                   ::  SO2_rate_pertur
...
contains
  ...
  subroutine LE_Chem_Step( c, aerh2o, t ,deltat, lrestart, status )
    ...
    use LE_Data      , only : LE_Data_GetPointer
     ...
     ! Santiago, traer la variable rk_pertur desde el lekf_driver, algo como, ojo hacer pública la variable en lekf_driver
    !real,allocatable   ::  rk_pertur(nx,ny)
    use LE_Chem_Work          , only : ireac_R71
    ...
    do k = 1, nz
      ...
      !$OMP   shared ( i_SO4a_f, i_SO4a_c, i_Na_f1, i_Na_f2, i_Na_c3, i_Na_c2, i_Na_c1 ) &
      ...
      do j = 1, ny
        do i = 1, nx
          ...
          if ( i_Na_f2 > 0 ) then
            ...
            conc_aer = max( c(i,j,k,i_Na_f2) * Na_to_seasalt, 1.e-6 )
            ! relative increase of aerosol radius due to water uptake ;
          ...
          if ( i_Na_c3 > 0 ) then
            ...
            conc_aer = max( c(i,j,k,i_Na_c3) * Na_to_seasalt, 1.e-6 )
            ! relative increase of aerosol radius due to water uptake ;
            ! based on relative humidity around the seasalt particles:
            finc = increment_factor_rh( conc_aer, rho_seasalt, rh(i,j,k) )
            !
          ...
          ! Santiago aca
          !Tener cuidado de no usar cuando no se está corriendo el lekf con perturbacions en el SO2, 
          !Se puede pensar poner en el lekf_driver una variable que sea 0 cuando no se usa y 1 cuando si
          !algo como
          if (SO2_rate_pertur) then
            rk(ireac_R71) = rk(ireac_R71) * rk_pertur(i,j)
            if ((i==10) .and. (j==10)) then
                write(*,*) 'Santiago le chem ======'
                write(*,*) rk(ireac_R71)
            end if
          end if
          ...
          if ( i_na_f1  > 0 ) ch(i_na_f1 ) = ch(i_na_f1 ) / specmolm(i_na_f1 ) * Avog * 1.0e-15  ! mlc/cm3
          if ( i_na_f2  > 0 ) ch(i_na_f2 ) = ch(i_na_f2 ) / specmolm(i_na_f2 ) * Avog * 1.0e-15  ! mlc/cm3
          if ( i_na_c1  > 0 ) ch(i_na_c1 ) = ch(i_na_c1 ) / specmolm(i_na_c1 ) * Avog * 1.0e-15  ! mlc/cm3
          if ( i_na_c2  > 0 ) ch(i_na_c2 ) = ch(i_na_c2 ) / specmolm(i_na_c2 ) * Avog * 1.0e-15  ! mlc/cm3
          if ( i_na_c3  > 0 ) ch(i_na_c3 ) = ch(i_na_c3 ) / specmolm(i_na_c3 ) * Avog * 1.0e-15  ! mlc/cm3
...
!          else
            ...
            call TWOSTEP( nspec, ts, min2sec * deltat, min( min2sec * deltat, dtmin ), dtmax, &
                          ch, Q, &
#ifdef with_labeling
...
!          end if   ! check timestep
          ...
          if ( i_na_f1  > 0 ) ch(i_na_f1 ) = ch(i_na_f1 ) * specmolm(i_na_f1 ) / Avog * 1.0e15  ! ug/m3
          if ( i_na_f2  > 0 ) ch(i_na_f2 ) = ch(i_na_f2 ) * specmolm(i_na_f2 ) / Avog * 1.0e15  ! ug/m3
          if ( i_na_c1  > 0 ) ch(i_na_c1 ) = ch(i_na_c1 ) * specmolm(i_na_c1 ) / Avog * 1.0e15  ! ug/m3
          if ( i_na_c2  > 0 ) ch(i_na_c2 ) = ch(i_na_c2 ) * specmolm(i_na_c2 ) / Avog * 1.0e15  ! ug/m3
          if ( i_na_c3  > 0 ) ch(i_na_c3 ) = ch(i_na_c3 ) * specmolm(i_na_c3 ) / Avog * 1.0e15  ! ug/m3
