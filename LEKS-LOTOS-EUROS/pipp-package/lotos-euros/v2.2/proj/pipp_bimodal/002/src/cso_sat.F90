...
module CSO_Sat
  ...
  type T_Mapping
    ...
    integer, allocatable              ::  map_n(:)      ! (npix)
...
contains
  ...
  subroutine Mapping_GetPixel( self, ipix, status, &
    ...
    if ( present(nw) ) then
      ! check ...
      if ( .not. allocated(self%map_n) ) then
        !! testing ...
  ...
  subroutine CSO_Sat_Data_Init( self, rcF, rcbase, filename, status )
    ...
    if ( self%with_track .and. csoc%root ) then
      ...
      call self%pd%NcInit( 'pixel', '', self%ncid, 'pixel', status, xtype='int', glb=.true. )
      IF_NOT_OK_RETURN(status=1)
  ...
  subroutine CSO_Sat_Data_Read( self, rcF, rcbase, status )
    ...
    ! gather on root:
    call csoc%GatherV( self%iglb, self%iglb_all, status, nloc=self%npix )
    IF_NOT_OK_RETURN(status=1)
...
!Santiago add read name_error and error variable from CSO file
  subroutine CSO_Sat_Data_GetPixel( self, ipix, status, &
                                            glbid, lon, lat, clons, clats, &
                                            name, name_error, value, profile, obs_error, &
    ...
    character(len=*), intent(in), optional        ::  name
    character(len=*), intent(in), optional        ::  name_error ! Name of error variable
    real, pointer, optional                       ::  value
    real, pointer, optional                       ::  profile(:)
    real, pointer, optional                       ::  obs_error(:)
    integer, intent(out), optional                ::  nw
    integer, pointer, optional                    ::  ii(:)   ! (nmap)
    integer, pointer, optional                    ::  jj(:)   ! (nmap)
    real, pointer, optional                       ::  ww(:)   ! (nmap)
    ...
    if ( present(obs_error) ) then
      ! get pointer:
      call self%pd%GetData( status, name=name_error, data1=data1 )
      IF_NOT_OK_RETURN(status=1)
      ! assign:
      obs_error => data1(:,ipix)
    end if
  ...
  subroutine CSO_Sat_Data_PutOut( self, filename, status )
    ...
    if ( self%nout > 0 ) then
      ...
      ! info ..
      write (csol,'(a,":   put out ",i0," pixels ...")') rname, self%nout; call csoPr
      ...
      if ( self%putout_mapping ) then
        ...
        call self%mapping%Collect( self%npix, self%iout_all, self%nout, self%iout_glb, status )
        IF_NOT_OK_RETURN(status=1)
      end if
      ...
      if ( csoc%root ) then
        ...
        call self%pd%NcDef( ncid, dimid_pixel, status )
        IF_NOT_OK_RETURN(status=1)
        ...
        call self%pd%NcPutGlbSelect( ncid, self%iout_glb, status )
        IF_NOT_OK_RETURN(status=1)
      ...
      call self%pd%NcPutGather( ncid, self%iout_all, .false., status )
      IF_NOT_OK_RETURN(status=1)
  ...
  subroutine CSO_Sat_Data_SetupExchange( self, status )
    ...
    integer                          ::  nex_max
    integer                          ::  nex
    integer, allocatable             ::  exchi(:,:,:)   ! (np,2,nex_max)
    integer                          ::  i1, i2
    integer                          ::  k
    ...
    if ( csoc%npes > 1 ) then
      ...
      ! storage processor id's per pixel contribution, needed on root only:
      if ( csoc%root ) then
        allocate( idi_all(np,self%npix_all), source=-999, stat=status )
        IF_NOT_OK_RETURN(status=1)
      else
      ...
      if ( csoc%root ) then
        ...
        do ipix_all = 1, self%npix_all
          ! global index:
          iglb = self%iglb_all(ipix_all)
          ! increase counter:
          qn(iglb) = qn(iglb) + 1
          ! check ...
        ...
        ! exchange parameters:
        !   local pid, local ipix, remote pid, remote ipix
        ! maximum storage with exchange info:
        allocate( exchi(np,2,nex_max), source=-999, stat=status )
        IF_NOT_OK_RETURN(status=1)
            ...
            do iglb = 1, self%nglb
              ...
              if ( qn(iglb) > 1 ) then
                ...
                if ( (i1 > 0) .and. (i2 > 0) ) then
                  ...
                  nex = nex + 1
                  ! fill info:
                  exchi(:,1,nex) = qidi(:,i1,iglb)
                  exchi(:,2,nex) = qidi(:,i2,iglb)
                end if
              end if ! qn > 1
            end do ! iglb
            ! info ...
...
!  end subroutine CSO_Sat_Data_GetRandom
  ...
  subroutine CSO_Sat_State_GetPixel( self, sdata, ipix, status, &
    ...
    if ( present(y) ) then
      ! get pointer: Santiago Changed name=y by name=ys
      call self%pd%GetData( status, name='ys', data1=ydata )
      IF_NOT_OK_RETURN(status=1)
      ! assign:
      y => ydata(:,ipix)
    end if
  ...
  subroutine CSO_Sat_State_PutOut( self, sdata, filename, status )
    ...
    if ( sdata%nout > 0 ) then
      ...
      if ( csoc%root ) then
        ...
        ! pixel dimension:
        status = NF90_Def_Dim( ncid, 'pixel', sdata%nout, dimid_pixel )
        IF_NF90_NOT_OK_RETURN(status=1)
