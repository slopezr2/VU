...
contains
  ...
  subroutine LE_Output_Rad_Init( leo, rcF, rckey, typ, name, state, status )
    ...
    !    
    !Santiago added the  if (.not. allocated(aod_keylist)) statements 
   if (.not. allocated(aod_keylist)) allocate( aod_keylist(self%n))  
   if (.not. allocated(ssa_col_keylist)) allocate( ssa_col_keylist(self%n)) 
   if (.not. allocated(extinction_keylist)) allocate( extinction_keylist(self%n))     
   if (.not. allocated(tau_keylist)) allocate( tau_keylist(self%n)) 
   if (.not. allocated(ssa_keylist)) allocate( ssa_keylist(self%n)) 
   if (.not. allocated(asy_keylist)) allocate( asy_keylist(self%n))         
