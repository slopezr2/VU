...
!    if ( present(name) ) name = trim(self%dimnames(id))
  ...
  subroutine PixelDatas_ApplyFormulas( self, pd, status )
    ...
    real, pointer                   ::  AEC_data(:,:)  ! (nretr,nlayer,npix)
    real, pointer                   ::  AOD_data(:,:)  ! (nretr,npix)
    real, pointer                   ::  AEx_data(:,:)  ! (nretr,npix)
    real, pointer                   ::  SSA_data(:,:)  ! (nretr,npix)
    real, pointer                   ::  AAOD_data(:,:)  ! (nretr,npix)
    real, pointer                   ::  SFC_data(:,:)  ! (nretr,npix)
    character(len=64)          ::  A_units,AOD_units,AEx_units,SSA_units,AAOD_units,SFC_units, AEC_units
    ...
    do id = 1, self%n
      ...
      select case ( trim(p%formula) )
          ...
          select case ( trim(mod_conc_units)//' -> '//trim(y_units) )
              ...
              case ( 'ppb -> mol m-2' )  
                ...
                do ipix = 1, npix
                  ...
                  ! scale model pressures to have same surface pressure as pixel: Santiago
            ...
            case ( 'm-1 -> m-1' )  
             ! loop over pixels:
                do ipix = 1, npix
                  ...
                  !   sum_i  c(i)  dp(i)
                  !          ppb    Pa
                  call ProfileMapping%Apply_WeightedMean( mod_conc_data(:,ipix), hx, status )
                  IF_NOT_OK_RETURN(status=1)
                  ! unit conversion:
                  y_data(:,ipix) = hx
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
         ...
         !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        !~ just AEC Aerosol Extinction Coefficient 
        case ( 'AEC' )
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~        
        ! pointer to target array:
          call self%GetData( status, id=id, data1=y_data, units=y_units )
          IF_NOT_OK_RETURN(status=1)
          ! get pointers to source arrays:
          call self%GetFormulaData( p%formula_terms, 'AEC', status, pd=pd, data1=AEC_data, units=AEC_units )
          npix = size(AEC_data,2)
          ! apply:
          do ipix = 1, npix
            ! fill:
            y_data(:,ipix) = AEC_data(:,ipix)
          end do
        ...
        ! pointer to target array:
          ...
          IF_NOT_OK_RETURN(status=1)
          ! get pointers to source arrays:
          call self%GetFormulaData( p%formula_terms, 'AOD', status, pd=pd, data1=AOD_data, units=AOD_units )
          npix = size(AOD_data,2)
          ! apply:
          do ipix = 1, npix
            ! fill:
            y_data(:,ipix) = AOD_data(:,ipix)
          end do
        ...
        ! pointer to target array:
          ...
          IF_NOT_OK_RETURN(status=1)
          ! get pointers to source arrays:
          call self%GetFormulaData( p%formula_terms, 'SFC', status, pd=pd, data1=SFC_data, units=SFC_units )
          npix = size(SFC_data,2)
          ! apply:
          do ipix = 1, npix
            ! fill:
            y_data(:,ipix) = SFC_data(:,ipix)
          end do
        ...
        ! pointer to target array:
          ...
          IF_NOT_OK_RETURN(status=1)
          ! get pointers to source arrays:
          call self%GetFormulaData( p%formula_terms, 'AAOD', status, pd=pd, data1=AAOD_data, units=AAOD_units )
          npix = size(AAOD_data,2)
          ! apply:
          do ipix = 1, npix
            ! fill:
            y_data(:,ipix) = AAOD_data(:,ipix)
          end do
        ...
        ! pointer to target array:
          ...
          IF_NOT_OK_RETURN(status=1)
          ! get pointers to source arrays:
          call self%GetFormulaData( p%formula_terms, 'AEx', status, pd=pd, data1=AEx_data, units=AEx_units )
          select case ( trim(AEx_units) )
                case('aeronet')
                    lambda1=0.440
                    lambda2=0.870
                case('modis')
                ...
                case('polder')
                    ...
                    lambda2=0.865
                case default
                    write (*,'("unsupported instrument name for angstrom exponent`",a,"`")') trim(AEx_units)
                                 TRACEBACK; status=1; return
            end select
          call swbands%FindBand( lambda1, j_angstrom1, status )
          IF_NOT_OK_RETURN(status=1)
          call swbands%FindBand( lambda2, j_angstrom2, status )
          IF_NOT_OK_RETURN(status=1)
  
          lambda1 = swbands%lambda( j_angstrom1)
          lambda2 = swbands%lambda( j_angstrom2)
          npix = size(AEx_data,2)
          ! apply:
          do ipix = 1, npix
            ! fill: ssa(ix,iy,wband) = - log( tau1 / tau2 ) / log( lambda1 / lambda2 )
            y_data(:,ipix) = -log(AEx_data(1,ipix)/AEx_data(2,ipix))/ log( lambda1 / lambda2 )
          end do
        ...
        ! pointer to target array:
          ...
          IF_NOT_OK_RETURN(status=1)
          ! get pointers to source arrays:
          call self%GetFormulaData( p%formula_terms, 'SSA', status, pd=pd, data1=SSA_data, units=SSA_units )
          npix = size(SSA_data,2)
          do ipix = 1, npix
            ! fill: ssa(ix,iy,wband) = 1-(AAOD(ix,iy,wband)/AOD(ix,iy,wband))
            y_data(:,ipix) = 1-(SSA_data(2,ipix)/SSA_data(1,ipix))
          end do
