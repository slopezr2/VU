    ...
    do itracer = 1, self%ntracer
      ...
      if ( n > 0 ) then
        ...
        do i = 1, n
          ...
          ! get weight factor and name of variable (could be empty):
          call self%varsum(itracer)%GetElement( i, status, w=w, name=name )
          IF_NOTOK_RETURN(status=1)
          ! variable defined?
          if ( len_trim(name) > 0 ) then
        
            ! variable defined, get pointer to data;
            ! data should be defined with at least 1 halo cell horizontally
            ! and on layers up to "top":
            call self%varsum(itracer)%GetElement( i, status, pdata=pdata, units=units, &
          ...
          else ! no varible name in formula
        
            ! add weight only:
            do k = 1, nlev
              bc_west (:,k,ispec) = bc_west (:,k,ispec) + w
              bc_east (:,k,ispec) = bc_east (:,k,ispec) + w
              bc_south(:,k,ispec) = bc_south(:,k,ispec) + w
              bc_north(:,k,ispec) = bc_north(:,k,ispec) + w
            end do
            do k = nlev+1, nlev_top
              caloft(:,:,k,ispec) = caloft(:,:,k,ispec) + w
            end do
          
          end if ! variable name in formula
