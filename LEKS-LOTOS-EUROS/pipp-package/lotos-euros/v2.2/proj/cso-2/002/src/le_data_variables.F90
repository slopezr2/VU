...
contains
  ...
  subroutine Variables_GetVariable( self, name, status, V, &
    ...
    class(T_Variable), pointer, optional                  ::  V
  ...
  recursive subroutine Variables_Setup( self, varname, tref, t1, t2, status, nodeps )
    ...
    use GO, only : TDate, operator(+), operator(-), operator(*), wrtgol
    ...
    !! info ...
    !call wrtgol( rname//': setup variable "'//trim(varname)//'" ', t1, ' - ', t2 ); call goPr
    ...
    if ( status == 0 ) then
      ...
      if ( len_trim(V%routine) > 0 ) then
        ...
        select case ( trim(V%routine) )
          ...
          case ( 'CalcKH' )
            ! arguments: ( uv_u, uv_v, dx,dy )
            call Variables_Calc_KH( self, trim(varname), V%deps(1:V%ndep), tmid, status )
            IF_NOTOK_RETURN(status=1)
          ...
          case ( 'VolumeFlux_w0' )
            ...
            IF_NOTOK_RETURN(status=1)
          !~ estimate of vertical wind speed (m/s) in cell center, derived from volume fluxes:
          case ( 'VerticalWind' )
            ! arguments: ( wflux, area )
            call Variables_Calc_VerticalWind( self, trim(varname), V%deps(1:V%ndep), tmid, status )
            IF_NOTOK_RETURN(status=1)
          ...
          ! ~ Bottom windspeed
          case ( 'WindSpeedProfile' )
            ! arguments: t, u, Q0v
            call Variables_Calc_WindSpeedProfile( self, trim(varname), V%deps(1:V%ndep), tmid, status )
            IF_NOTOK_RETURN(status=1)
  ...
  subroutine Variables_Calc_KH( self, varname, args, t, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/Variables_Calc_KH'

    ! argument names:
    integer, parameter        ::  i_uv_u = 1
    integer, parameter        ::  i_uv_v = 2
    integer, parameter        ::  i_dx = 3
    integer, parameter        ::  i_dy = 4
    ...
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                              check_units='1/s', lbo=lbo, ubo=ubo  )
    IF_NOTOK_RETURN(status=1)
    
    ! input variable, extract units and shape:
    call self%GetValidPtr( args(i_uv_u), t,t, u_u, v_u, status ) !, &
!                             lbo=lbo_u, ubo=ubo_u )
    IF_NOTOK_RETURN(status=1)
    
    call self%GetValidPtr( args(i_uv_v), t,t, u_v, v_v, status ) !, &
!                             lbo=lbo_v, ubo=ubo_v )
    IF_NOTOK_RETURN(status=1)
    call self%GetValidPtr( args(i_dx), t,t, dx, status ) !, &
    IF_NOTOK_RETURN(status=1)
    call self%GetValidPtr( args(i_dy), t,t, dy, status ) !, &
    IF_NOTOK_RETURN(status=1)
    ...
    ! Calculate diffusion coefficients for cell centers    
    do k = lbo(3), ubo(3)
    do j = lbo(2), ubo(2)
    do i = lbo(1), ubo(1)
      dudx = (u_u(i,j,k)-u_u(i-1,j,k)) / dx(i,j,1)
      dudy = (u_v(i,j,k)-u_v(i,j-1,k)) / ( 0.5*(dy(i,j-1,1)+dy(i,j,1)) )
      
      dvdx = (v_u(i,j,k)-v_u(i-1,j,k)) / dx(i,j,1)
      dvdy = (v_v(i,j,k)-v_v(i,j-1,k)) / ( 0.5*(dy(i,j-1,1)+dy(i,j,1)) )
  ...
  end subroutine Variables_Calc_KH
...
!  end subroutine Variables_Calc_MixlayerHAltBounds
  ...
  subroutine Variables_Calc_Total( self, varname, args, t, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/Variables_Calc_Total'
  ...
  !
  ! Compute something like a vertical wind speed (m/s) in mid of layer
  ! from vertical volume fluxes (m3/s) through interfaces and cell area (m2):
  !
  !     !                            m3/s                           m2
  !     w(:,:,k)  =  ( wflux(:,:,k-1/2) + wflux(:,:,k+1/2) )/2.0 / area  ! m
  ! 

  subroutine Variables_Calc_VerticalWind( self, varname, args, t, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/Variables_Calc_VerticalWind'
    
    ! argument names:
    integer, parameter        ::  i_wflux = 1
    integer, parameter        ::  i_area  = 2
    ...
    ! pointer to target field, check units, get target shape:
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                             check_units='m/s', lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( w, t, status, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)

    ! input array, check units and shape (half levels):
    call self%GetValidPtr( args(i_wflux), t,t, wflux, status, &
                              check_units='m3/s', &
                              check_lbo=lbo+(/0,0,-1/), check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! input array, check units and shape (surface):
    call self%GetValidPtr( args(i_area), t,t, area, status, &
                              check_units='m2', &
                              check_lbo=(/lbo(1),lbo(2),1/), check_ubo=(/ubo(1),ubo(2),1/) )
    IF_NOTOK_RETURN(status=1)
    ...
    do k = lbo(3), ubo(3)
      do j = lbo(2), ubo(2)
        do i = lbo(1), ubo(1)
          ...
          !                         m3/s                        m2
          w(i,j,k) = ( wflux(i,j,k-1) + wflux(i,j,k) )/2.0 * area(i,j,1)  ! m
        end do ! i
      end do ! j
    end do ! i
  ...
  end subroutine Variables_Calc_VerticalWind
  ...
  !
  !  Scale parameters q*;
  !  this has the same function in vertical profiles 
  !  for moisture as u* has for velocity.
  !
  !  From eq. (3.7):
  !    rho u* q* = Jq   (~slhf)
  !
  ...
  subroutine Variables_Calc_ScaleParameterSpecificHumidity( self, varname, args, t, status )
    ...
    use Binas       , only : lvap   ! latent heat of evaporation [J kg-1]
    ...
    ! pointer to target field given the name, check units, get index space::
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                             check_units='kg/kg', lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    
    ! pointer to input data, check index space:
    call self%GetValidPtr( args(i_slhf), t,t, slhf, status, &
                               check_units='J/m2/s', check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space:
    call self%GetValidPtr( args(i_zust), t,t, zust , status, &
                               check_units='m/s', check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space (should have at least surface index):
    call self%GetValidPtr( args(i_dens), t,t, dens , status, &
                               check_units='kg/m3', check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)

    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( q_star, t, status, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ...
    do j = lbo(2), ubo(2)
      do i = lbo(1), ubo(1)
        ...
        !                   J/m2/s    / J/(kg water) / (kg air)/m3 /  m/s
        q_star(i,j,1) = - slhf(i,j,1) /     lvap     / dens(i,j,1) / zust(i,j,1)  ! (kg water)/(kg air)
      end do ! i
    end do ! j
  ...
  !  Scale parameters s*;
  !  this has the same function in vertical profiles 
  !  for heat as u* has for velocity.
  !
  !  From eq. (3.7):
  ...
  subroutine Variables_Calc_ScaleParameterDryStaticEnergy( self, varname, args, t, status )
    ...
    ! pointers to input data, check index space:
    call self%GetValidPtr( args(i_sshf), t,t, sshf, status, &
                              check_units='J/m2/s', &
                               check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space:
    ...
    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( s_star, t, status, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ...
    do j = lbo(2), ubo(2)
      do i = lbo(1), ubo(1)
        ...
        s_star(i,j,1) = - sshf(i,j,1) /  dens(i,j,1) / zust(i,j,1)  ! J/(kg air)
      end do ! i
    end do ! j
  ...
  !               -------     k-1/2       (k-1)
  !
  ! Gradient of u at half level:
  !
  !     du            u_{k+1} - u_{k}
  !    (--)_{k+1/2} = ---------------
  !     dz               dz_{k+1/2}
  !
  ! with distance between mid of layers, here from half-level altitudes:
  !
  !    dz_{k+1/2}  = [ z_{k+1/2} + z_{k+1+1/2} ]/2 - [ z_{k-1/2} + z_{k+1/2} ]/2
  !                = [ z_{k+1+1/2} - z_{k-1/2} ]/2
  !
  ! Arguments:
  !    uu  (1:n)  : values at mid levels
  !    halt(0:n)  : altitudes of half levels
  !
  ! Return values:
  !    duu_dz(0:n) : gradients at half levels, zero at 0 and n [uu]/[halt]
  !
  ...
  subroutine Variables_Calc_HalfLevelGradient( self, varname, args, t, status )
    ...
    real, pointer                       ::  halt(:,:,:)
    integer                             ::  lbo_hlv(3), ubo_hlv(3)
    integer                             ::  lbo_tmp(3), ubo_tmp(3)
    character(len=32)                   ::  uu_units, halt_units
    class(T_Instant_Field), pointer     ::  iF
    real, pointer                       ::  duu_dz(:,:,:)
    real                                ::  dz
    integer                             ::  i, j, l
    ...
    ! pointer to input data, here only to get units:
    call self%GetValidPtr( args(i_uu), t,t, uu, status, units=uu_units )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, here only to get units and half-level index space:
    call self%GetValidPtr( args(i_halt), t,t, halt, status, &
                              units=halt_units, lbo=lbo_tmp, ubo=ubo_tmp )
    IF_NOTOK_RETURN(status=1)
    
    ! pointer to target field given the name, check units, get index space (horizontal is used):
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                                check_units=trim(uu_units)//'/'//trim(halt_units), &
                                lbo=lbo_hlv, ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
    ! set index space for half-levels:
    lbo_hlv(3) = lbo_tmp(3)
    ubo_hlv(3) = ubo_tmp(3)
    
    ! pointer to input data, check index space (half levels):
    call self%GetValidPtr( args(i_halt), t,t, halt, status, &
                              check_lbo=lbo_hlv, check_ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space (mid levels):
    call self%GetValidPtr( args(i_uu), t,t, uu, status, &
                               check_lbo=lbo_hlv+(/0,0,1/), check_ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)

    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( duu_dz, t, status, lbo=lbo_hlv, ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
    
    ! loop over cells:
    do j = lbo_hlv(2), ubo_hlv(2)
      do i = lbo_hlv(1), ubo_hlv(1)
        ! loop over half layers:
        do l = lbo_hlv(3), ubo_hlv(3)

          if ( (l == lbo_hlv(3)) .or. (l == ubo_hlv(3)) ) then
            ! no gradient over top and surface:
            duu_dz(i,j,l) = 0.0
          else
            ! distance between mid of layers:
            dz = 0.5*( halt(i,j,l+1) - halt(i,j,l-1) )
            ! gradient at half level:
            duu_dz(i,j,l) = ( uu(i,j,l+1) - uu(i,j,l) ) / dz
          end if
  ...
  !
  ! Absolute windspeed profile,
  ! at the bottom model layer following eq. (3.17):
  !
  ...
  !
  ! Arguments:
  !    wspd_n    : windspeed   at model layers
  !    T_n       : temperature at model layers
  !    Q0v       : virtual temperature flux in the surface layer [K m/s ?]
  !

  subroutine Variables_Calc_WindSpeedProfile( self, varname, args, t, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/Variables_Calc_WindSpeedProfile'
    
    ! argument names:
    integer, parameter        ::  i_temp  = 1
    integer, parameter        ::  i_wspd  = 2
    integer, parameter        ::  i_Q0v   = 3
    ...
    ! pointer to input data, here to get index space for levels:
    call self%GetValidPtr( args(i_temp), t,t, temper, status, &
                               lbo=lbo_tmp, ubo=ubo_tmp )
    IF_NOTOK_RETURN(status=1)
    
    ! pointer to target field given the name, get index space (horizontal is used):
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                             check_units='m/s', lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! reset to input levels:
    lbo(3) = lbo_tmp(3)
    ubo(3) = ubo_tmp(3)
    ! surface bound checks
    lbo_sfc = (/lbo(1),lbo(2), 1/)
    ...
    ! pointer to input data, check units, check index space:
    call self%GetValidPtr( args(i_temp), t,t, temper, status, &
                               check_units='K', &
                               check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check units, check index space:
    call self%GetValidPtr( args(i_wspd), t,t, wspd, status, &
                               check_units='m/s', &
                               check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space:
      ...
      do i = lbo(1), ubo(1)
        do l = lbo(3), ubo(3)
          ...
          if ( l == lbo(3) ) then
            ...
            values(i,j,l) = sqrt( wspd(i,j,l)**2 + wstar**2 )
          else
            ! absolute windspeed:
            values(i,j,l) = wspd(i,j,l)
          end if
  ...
  end subroutine Variables_Calc_WindSpeedProfile
  ...
  subroutine Variables_Calc_VirtualTemperatureFlux( self, varname, args, t, status )
    ...
    ! pointer to target field given the name, get index space:
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
    ...
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space:
    call self%GetValidPtr( args(i_sstar), t,t, sstar, status, &
                               check_units='J/kg', &
                               check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space:
    call self%GetValidPtr( args(i_temp), t,t, tsurf, status, &
                               check_units='K', &
                               check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ! pointer to input data, check index space:
    call self%GetValidPtr( args(i_qstar), t,t, qstar, status, &
                               check_units='kg/kg', &
                               check_lbo=lbo, check_ubo=ubo )
    IF_NOTOK_RETURN(status=1)

    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( Q0v, t, status, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ...
    do j = lbo(2), ubo(2)
      do i = lbo(1), ubo(1)    
        ...
        Q0v(i,j,1) = zust(i,j,1)*sstar(i,j,1)/cp_air + eps1*tsurf(i,j,1)*zust(i,j,1)*qstar(i,j,1)  ! K m/s

        ! avoid Q0v==0 which leads to division by zero; 
        ! round small numbers to negatve threshold (which gives L>0, stable conditions):
        if ( abs(Q0v(i,j,1)) < abs(Q0v_maxneg) ) then
          Q0v(i,j,1) = Q0v_maxneg
        end if
  ...
  subroutine Variables_Calc_DryStaticEnergy( self, varname, args, t, status )
    ...
    ! pointer to input data, here only needed for index space of levels:
    call self%GetValidPtr( args(i_temp), t,t, temper, status, &
                               lbo=lbo_tmp, ubo=ubo_tmp  )
    IF_NOTOK_RETURN(status=1)
    ...
    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( dse, t, status, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ...
    do j = lbo(2), ubo(2)
      do i = lbo(1), ubo(1)
        ...
        do l = lbo(3), ubo(3)
          ...
          ! compute:
          !               m/s2   m   J/kg/K    K
          dse(i,j,l) = grav * z + cp_air * temper(i,j,l)   ! [J/kg]
        end do ! l
  ...
  subroutine Variables_Calc_RichardsonNumber( self, varname, args, t, status )
    ...
    ! pointer to input data, here to get index space of half-levels:
    call self%GetValidPtr( args(i_halt), t,t, halt, status, &
                               lbo=lbo_tmp, ubo=ubo_tmp )
    IF_NOTOK_RETURN(status=1)
    
    ! pointer to target field given the name, check units, get index sapce (horizontal needed):
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                             check_units='1', lbo=lbo_hlv, ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
    ! reset index space for half-level fields:
    lbo_hlv(3) = lbo_tmp(3)
    ubo_hlv(3) = ubo_tmp(3)
    ! required index space for level fields:
    lbo = lbo_hlv + (/0,0,1/)
    ubo = ubo_hlv
    ...
    call self%GetValidPtr( args(i_halt), t,t, halt, status, check_units='m', &
                               check_lbo=lbo_hlv, check_ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
    ! input variable, check units, get index space (levels are used):
    ...
    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( Ri, t, status, lbo=lbo_hlv, ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
      ...
      do i = lbo_hlv(1), ubo_hlv(1)       
        ...
        do l = lbo_hlv(3), ubo_hlv(3)
          ...
          if ( (l == lbo_hlv(3)) .or. (l == ubo_hlv(3)) ) then     
            ...
            delta_u2 =  ( u(i,j,l+1) - u(i,j,l) )**2 + ( v(i,j,l+1)-v(i,j,l) )**2  ! m2/s2 
            ...
            ! distance between mid of layers:
            !   dz = [halt(l)+halt(l+1)]/2 - [halt(l-1)+halt(l)]/2
            !      = [ halt(l+1) - halt(l-1) ]/2
            dz = 0.5 * ( halt(i,j,l+1) - halt(i,j,l-1) )
                
            ! combine:
            !           m/s2   m     1    /   m2/s2
            Ri(i,j,l) = grav * dz * ds_cpT / delta_u2  ! 1
          end if
  ...
  ! HalfLevel Kz IFS
  !   output: kz_ifs
  !   input : duudz, Ri, monin, halt, blh, ustar
  !
  ...
  subroutine Variables_Calc_HalfLevelKzIFS( self, varname, args, t, status )
    ...
    ! argument names:
    integer, parameter        ::  i_duudz  = 1
    integer, parameter        ::  i_ri     = 2
    integer, parameter        ::  i_monin  = 3
    integer, parameter        ::  i_halt   = 4
    integer, parameter        ::  i_blh    = 5
    integer, parameter        ::  i_ustar  = 6
    ...
    ! pointer to input data, here to obtain index space for half levels:
    call self%GetValidPtr( args(i_halt), t,t, halt, status, &
                               lbo=lbo_tmp, ubo=ubo_tmp )
    IF_NOTOK_RETURN(status=1)
        
    ! pointer to target field given the name, check units, get index space (horizontal):
    call self%GetVariable( trim(varname), status, Instant_Field=iF, &
                             check_units='m2/s', lbo=lbo_hlv, ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
    ! levels might be undefined yet, copy from input:
    lbo_hlv(3) = lbo_tmp(3)
    ubo_hlv(3) = ubo_tmp(3)
    ! surface bounds:
    lbo_sfc = (/lbo_hlv(1),lbo_hlv(2), 1/)
    ubo_sfc = (/ubo_hlv(1),ubo_hlv(2), 1/)
    ...
    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( Kz, t, status, lbo=lbo_hlv, ubo=ubo_hlv )
    IF_NOTOK_RETURN(status=1)
      ...
      do i = lbo_hlv(1), ubo_hlv(1)
        ...
        do l = lbo_hlv(3), ubo_hlv(3)
          ...
          if ( l == lbo_hlv(3) ) then
            ...
            !! testing ...
            !call ugg%GetLocation( -3.7, 40.4, qi, qj, status, quiet=.true. )
            !if ( status == 0 ) then
            !  qdebug = (i == qi) .and. (j == qj) .and. (l==lbo(3)+1)
            !else
            !  qdebug = .false.
            !end if
            !if ( qdebug ) then
            !  write (gol,*) 'qqq1 i,j = ', qi, qj, l; call goPr
            !  write (gol,*) '  q1 temper = ', temper(i,j,l); call goPr
            !  write (gol,*) '  q1 duudz = ', duudz(i,j,l); call goPr
            !  write (gol,*) '  q1 ri    = ', ri(i,j,l); call goPr
            !  write (gol,*) '  q1 monin = ', monin(i,j,1); call goPr
            !  write (gol,*) '  q1 z     = ', halt(i,j,l)-halt(i,j,lbo_hlv(3)); call goPr
            !  write (gol,*) '  q1 blh   = ', blh(i,j,1); call goPr
            !  write (gol,*) '  q1 ustar = ', ustar(i,j,1); call goPr
            !end if
            
            ! Kz at half levels;
            !  z [m] is height above surface, comput from halt (altitude)
            call HalfLevelKzIFS( duudz(i,j,l), Ri(i,j,l), monin(i,j,1), &
                                 halt(i,j,l)-halt(i,j,lbo_hlv(3)), blh(i,j,1), ustar(i,j,1), &
                                 Kz(i,j,l), &
                                 status )!, debug=qdebug )
            IF_NOTOK_RETURN(status=1)
            ...
            !! testing ..
            !if ( qdebug ) then
            !  write (gol,*) '  q1 Kz    = ', ustar(i,j,1); call goPr
            !end if

          end if  ! surface or half-levels
