...
contains
  ...
  subroutine AerosolModes_Init( self, status )
    ...
    call self%aerosol(i_aerosol)%Init(   2*0.2e-6  ,  1.59  , status ) ! Sakai 2010 (we divide by 2 because RH~80 means radius times 2) and Quinn 2015
    IF_NOTOK_RETURN(status=1)
    
    ! ss coarse
    i_aerosol = n_aerosol+2
    ! initialize aerosol description:  diameter [m],  sigma [1]
    call self%aerosol(i_aerosol)%Init(  2*2.5e-6    ,  2.0  , status )  ! For bimodal test
    ...
    call self%aerosol(i_aerosol)%Init(   2*0.2e-6   ,  1.59  , status ) ! For bimodal test
    IF_NOTOK_RETURN(status=1)
