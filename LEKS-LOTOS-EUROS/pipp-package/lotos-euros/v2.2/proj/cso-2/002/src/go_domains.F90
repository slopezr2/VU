...
contains
  ...
  subroutine Domains_Init( self, cart_dims, glb_lbo, glb_ubo, gocd, status )
    ...
    ! check ...
    if ( product(cart_dims) /= self%goc%npes ) then
      write (gol,'("requested decomposition on ",i0," domains while number of processors is ",i0," ...")') &
                         product(cart_dims), self%goc%npes; call goErr
    ...
    call MPI_Cart_Create( self%goc%comm, self%ndim, self%cart_dims, self%cart_periods, .true., &
    ...
    ! storage:
    allocate( self%glbo(self%ndim,0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    allocate( self%gubo(self%ndim,0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    allocate( self%off(self%ndim,0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    allocate( self%lbo(self%ndim,0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    allocate( self%ubo(self%ndim,0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    allocate( self%shp(self%ndim,0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    allocate( self%n(0:self%goc%npes-1), stat=status )
    IF_NOT_OK_RETURN(status=1)
    ...
    if ( self%ndim == 1 ) then
      write (gol,'("  global: (",i0,":",i0,")")') self%glbo(1,self%goc%id), self%gubo(1,self%goc%id); call goPr
      write (gol,'("  local : (",i0,":",i0,")")') self%lbo(1,self%goc%id), self%ubo(1,self%goc%id); call goPr
      write (gol,'("  offset: ",i0)') self%off(:,self%goc%id); call goPr
      write (gol,'("  shape : ",i0)') self%shp(:,self%goc%id); call goPr
      write (gol,'("  size  : ",i0)') self%n(self%goc%id); call goPr
    else if ( self%ndim == 2 ) then
      write (gol,'("  global: (",i0,":",i0,",",i0,":",i0,")")') self%glbo(1,self%goc%id), self%gubo(1,self%goc%id), &
                                                                self%glbo(2,self%goc%id), self%gubo(2,self%goc%id); call goPr
      write (gol,'("  local : (",i0,":",i0,",",i0,":",i0,")")') self%lbo(1,self%goc%id), self%ubo(1,self%goc%id), &
                                                                self%lbo(2,self%goc%id), self%ubo(2,self%goc%id); call goPr
      write (gol,'("  offset: ",i0,",",i0)') self%off(:,self%goc%id); call goPr
      write (gol,'("  shape : ",i0,",",i0)') self%shp(:,self%goc%id); call goPr
      write (gol,'("  size  : ",i0)') self%n(self%goc%id); call goPr
    else
...
!    IF_NOT_OK_RETURN(status=1)
  ...
  subroutine Domains_Intersection( self, glbo, gubo, lbo, ubo, shp, status )
    ...
    if ( any( self%shp(:,self%goc%id) <= 0 ) ) then
      ...
      lbo = max( self%glbo(:,self%goc%id), glbo ) - self%off(:,self%goc%id)
      ubo = min( self%gubo(:,self%goc%id), gubo ) - self%off(:,self%goc%id)
      ! local shape:
      shp = ubo - lbo + 1
  ...
  subroutine Domains_Inside( self, ind, status )
    ...
    ! compare:
    if ( all(self%glbo(:,self%goc%id) <= ind) .and. all(ind <= self%gubo(:,self%goc%id)) ) then
      ! in domain:
      status = 0
    else
      ! outside ..
  ...
  subroutine Domains_ExchangeHaloX_3d_r( self, nh, field, status )
    ...
    x1 = self%lbo(1,self%goc%id)
    x2 = self%ubo(1,self%goc%id)
    ! other dimensions:
    ny = size(field,2)
    nz = size(field,3)
    ...
    if ( (x1-nh < lbound(field,1)) .or. (x2+nh > ubound(field,1)) ) then
      ...
      write (gol,'("does not cover expected index space (",i0,":",i0,")")') self%lbo(1,self%goc%id), self%ubo(1,self%goc%id); call goErr
      write (gol,'("with ",i0," halo (",i0,":",i0,")")') nh, self%lbo(1,self%goc%id)-nh, self%ubo(1,self%goc%id)+nh; call goErr
      TRACEBACK; status=1; return
    end if
    ...
    ! data type:
    call self%goc%GetDataType( 'real', kind(field), mpi_dtype, status )
    IF_NOT_OK_RETURN(status=1)
  ...
  subroutine Domains_ExchangeHaloY_3d_r( self, nh, field, status )
    ...
    ! short variables for local index spaces:
    y1 = self%lbo(2,self%goc%id)
    y2 = self%ubo(2,self%goc%id)
    ! other dimensions:
    nx = size(field,1)
    nz = size(field,3)
    ...
    if ( (y1-nh < lbound(field,2)) .or. (y2+nh > ubound(field,2))        ) then
      ...
      write (gol,'("does not cover expected index space (",i0,":",i0,")")') self%lbo(2,self%goc%id), self%ubo(2,self%goc%id); call goErr
      write (gol,'("with ",i0," halo (",i0,":",i0,")")') nh, self%lbo(2,self%goc%id)-nh, self%ubo(2,self%goc%id)+nh; call goErr
      TRACEBACK; status=1; return
    end if
  ...
  subroutine Domains_ExchangeHaloX_3d_l( self, nh, field, status )
    ...
    ! data type:
    call self%goc%GetDataType( 'logical', 0, mpi_dtype, status )
    IF_NOT_OK_RETURN(status=1)
...
!              self%lbo(1,self%goc%id), self%ubo(1,self%goc%id), self%lbo(2,self%goc%id), self%ubo(2,self%goc%id); call goErr
!      write (gol,'("with ",i0," halo (",i0,":",i0,",",i0,":",i0,")")') nh, &
!              self%lbo(1,self%goc%id)-1-nh, self%ubo(1,self%goc%id)+nh, self%lbo(2,self%goc%id), self%ubo(2,self%goc%id); call goErr
!      TRACEBACK; status=1; return
!    end if
!
!    ! storage:
...
!!      write (gol,*) 'rrr field(',self%off(1,self%goc%id)+i,') = ', field(i,10,1); call goPr
!!    end do
...
!              self%lbo(1,self%goc%id), self%ubo(1,self%goc%id), self%lbo(2,self%goc%id), self%ubo(2,self%goc%id); call goErr
!      write (gol,'("with ",i0," halo (",i0,":",i0,",",i0,":",i0,")")') nh, &
!              self%lbo(1,self%goc%id), self%ubo(1,self%goc%id), self%lbo(2,self%goc%id)-1-nh, self%ubo(2,self%goc%id)+nh; call goErr
!      TRACEBACK; status=1; return
!    end if
!
!    ! storage:
...
!  end subroutine Domains_ExchangeHaloY_3d_r
  ...
  subroutine ExchangeTest( self, status )
    ...
    ! grid size:
    nx = self%shp(1,self%goc%id)
    ny = self%shp(2,self%goc%id)
    ! halo:
    nh = 2
  ...
  subroutine Domains_Swap_2d_r8( self, input, doms, output, status )
    ...
    ! local input slab ?
    if ( all( self%shp(:,self%goc%id) > 0 ) ) then
      ! check ...
      if ( any( shape(input) /= self%shp(:,self%goc%id) ) ) then
        write (gol,'("shape of input (",i0,3(",",i0),") while expected (",i0,3(",",i0),")")') &
                      shape(input), self%shp(:,self%goc%id); call goErr
        TRACEBACK; status=1; return
      end if
    !else
    !  ! info ...
    ...
    ! local output slab ?
    if ( all( doms%shp(:,self%goc%id) > 0 ) ) then
      ! check ...
      if ( any( shape(output) /= doms%shp(:,self%goc%id) ) ) then
        write (gol,'("shape of output (",i0,3(",",i0),") while doms define (",i0,3(",",i0),")")') &
                      shape(output), doms%shp(:,self%goc%id); call goErr
        TRACEBACK; status=1; return
      end if
    !else
    !  ! info ...
    ...
    ! storage:
    allocate( sendbuf(self%n(self%goc%id)) )
    allocate( sendcounts(0:self%goc%npes-1) )
    allocate( sdispls   (0:self%goc%npes-1) )
    
    ! init displacement:
    displ = 0
    ! loop over target processes:
    do pid = 0, self%goc%npes-1
    ...
    ! check ...
    if ( displ /= self%n(self%goc%id) ) then
      write (gol,'("stored ",i0," values in sendbuf, but local input size is ",i0)') displ, self%n(self%goc%id); call goErr
      TRACEBACK; status=1; return
    end if
    ...
    ! storage:
    allocate( recvbuf(doms%n(self%goc%id)) )
    allocate( recvcounts(0:self%goc%npes-1) )
    allocate( rdispls   (0:self%goc%npes-1) )
    ...
    ! check ...
    if ( displ /= doms%n(self%goc%id) ) then
      write (gol,'("prepared ",i0," values in recvbuf, but local output size is ",i0)') displ, doms%n(self%goc%id); call goErr
      TRACEBACK; status=1; return
    end if
  ...
  subroutine Domains_GatherV_1d_i( self, sendbuf, n, recvbuf, status )
    ...
    if ( self%goc%root ) then
      allocate( recvcounts(0:self%goc%npes-1), stat=status )
      IF_NOT_OK_RETURN(status=1)
    else
    ...
    call self%goc%Gather( n, recvcounts, status )
    IF_NOT_OK_RETURN(status=1)
    ...
    if ( self%goc%root ) then
      ...
      IF_NOT_OK_RETURN(status=1)
      allocate( displs    (0:self%goc%npes-1), stat=status )
      IF_NOT_OK_RETURN(status=1)
...
#ifdef _MPI
    ! data type:
    call self%goc%GetDataType( 'real', kind(sendbuf), mpi_dtype, status )
    IF_NOT_OK_RETURN(status=1)
    ...
    call MPI_GatherV( sendbuf, n                 , mpi_dtype, &
                      ...
                      self%goc%root_id, self%goc%comm, status )
    IF_MPI_NOT_OK_RETURN(status=1)
#else
    !! info ...
    !write (gol,'("copy data ...")'); call goPr
  ...
  subroutine Domains_GatherV_2d_r( self, input, output, status )
    ...
    if ( n > 0 ) then
      ...
      !        n, self%lbo(1,self%goc%id),self%ubo(1,self%goc%id),self%lbo(2,self%goc%id),self%ubo(2,self%goc%id),&
      !           self%lbo(3,self%goc%id),self%ubo(3,self%goc%id); call goPr
      ! copy slab into buffer:
      sendbuf = reshape( input(self%lbo(1,self%goc%id):self%ubo(1,self%goc%id),&
                               self%lbo(2,self%goc%id):self%ubo(2,self%goc%id)), (/n/) )
    else
      ! dummy:
  ...
  subroutine Domains_GatherV_3d_r( self, input, output, status )
    ...
    if ( all( self%shp(:,self%goc%id) > 0 ) ) then
      ! check ...
      if ( any( (/nx,ny/) /= self%shp(:,self%goc%id) ) ) then
        write (gol,'("shape of input (",i0,",",i0,",:) while expected (",i0,",",i0,",:)")') &
                      nx,ny, self%shp(:,self%goc%id); call goErr
        TRACEBACK; status=1; return
      end if
    !else
    !  ! info ...
  ...
  subroutine Domains_AllGather_1d_i4( self, input, output, status )
    ...
    if ( n > 0 ) then
      ...
      !           self%lbo(3,self%goc%id),self%ubo(3,self%goc%id); call goPr
      ! copy slab into buffer:
      sendbuf = reshape( input(self%lbo(1,self%goc%id):self%ubo(1,self%goc%id)), (/n/) )
    else
      ! dummy:
      allocate( sendbuf(1) )
    end if
    ...
    allocate( recvbuf(gn) )
    allocate( recvcounts(0:self%goc%npes-1) )
    allocate( displs    (0:self%goc%npes-1) )
  ...
  subroutine Domains_AllGather_3d_c8( self, input, output, status )
    ...
    if ( n > 0 ) then
      ...
      ! copy slab into buffer:
      sendbuf = reshape( input(self%lbo(1,self%goc%id):self%ubo(1,self%goc%id),&
                               self%lbo(2,self%goc%id):self%ubo(2,self%goc%id),&
                               self%lbo(3,self%goc%id):self%ubo(3,self%goc%id)), (/n/) )
    else
      ! dummy:
      allocate( sendbuf(1) )
    end if
  ...
  subroutine Domains_Extract_1d_r8( self, input, output, status )
    ...
    if ( all( self%shp(:,self%goc%id) > 0 ) ) then
      ...
      ! check ...
      if ( any( shape(output) /= self%shp(:,self%goc%id) ) ) then
        write (gol,'("shape of output (",i0,") while expected local shape (",i0,")")') &
                      shape(output), self%shp(:,self%goc%id); call goErr
        TRACEBACK; status=1; return
      end if

      !! testing ..
      !print *, me, ': extract ', self%glbo(1,self%goc%id)-goff(1), self%gubo(1,self%goc%id)-goff(1)
      
      ! copy local slab:
      output = input(self%glbo(1,self%goc%id)-goff(1):self%gubo(1,self%goc%id)-goff(1))
