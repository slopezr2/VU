...
module GO_Comm
  ...
  type TgoComm
    ...
    integer                         ::  npes
    integer                         ::  id
    ! process included in this communicator?
    logical                         ::  enabled
    ! root?
    integer                         ::  root_id
    logical                         ::  root
  ...
  contains
    ...
    procedure   ::  InitGroup   =>  GO_Comm_InitGroup
...
#endif
    !
    procedure                       GO_Comm_SendAndRecv_i4
    procedure                       GO_Comm_SendAndRecv_i4_1d
...
contains
  ...
  subroutine GO_Comm_Init( self, status )
    ...
    use MPI_F08, only : MPI_Comm_Group
#endif
    ...
    ! group of all processors:
    call MPI_Comm_Group( self%comm, self%group, ierror=status )
    IF_MPI_NOT_OK_RETURN(status=1)
    ...
    ! is this process in this communicator?
    self%enabled = self%id >= 0
    ...
    ! set last id:
    self%last_id = self%npes-1
    ! last?
    self%last = self%id == self%last_id
  ...
  !
  ! Init communicator for sub-group of id's in parent communicator.
  !
  
  subroutine GO_Comm_InitGroup( self, parent, ids, status )

#ifdef _MPI
    use MPI_F08, only : MPI_Group_Incl
    use MPI_F08, only : MPI_Comm_Create
    use MPI_F08, only : MPI_INFO_NULL
    use MPI_F08, only : MPI_Comm_Size, MPI_Comm_Rank
#endif
    ...
    class(TgoComm), intent(out)       ::  self
    type(TgoComm), intent(in)         ::  parent
    integer, intent(in)               ::  ids(:)
    integer, intent(out)              ::  status

    ! --- const ----------------------------------

    character(len=*), parameter   ::  rname = mname//'/GO_Comm_InitGroup'
    ...
    ! not the communicator for world:
    self%world = .false.

#ifdef _MPI
    ! create sub-group for the specified id's;
    ! all processes should make a call, 
    ! but only those in the group with filled arguments:
    if ( any( parent%id == ids ) ) then
      call MPI_Group_Incl( parent%group, size(ids), ids, self%group, ierror=status )
      IF_MPI_NOT_OK_RETURN(status=1)
    else
      call MPI_Group_Incl( parent%group, 0, (/-999/), self%group, ierror=status )
      IF_MPI_NOT_OK_RETURN(status=1)
    end if
    ! create communicator:
    call MPI_Comm_Create( parent%comm, self%group, self%comm, ierror=status )
    IF_MPI_NOT_OK_RETURN(status=1)
    ! part of group?
    if ( any( parent%id == ids ) ) then
      ! get size:
  ...
  end subroutine GO_Comm_InitGroup
...
!  end subroutine GO_Comm_GetID
  ...
  subroutine GO_Comm_SendAndRecv_i4( self, value, source, dest, tag, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/GO_Comm_SendAndRecv_i4'
    ...
    if ( self%id == source ) then
      ...
      !! testing ...
      !write (gol,'("  x    send integer (tag ",i0,")")') tag; call goPr
      ...
      !! testing ...
      !write (gol,'("  x    recv integer (tag ",i0,")")') tag; call goPr
  ...
  end subroutine GO_Comm_SendAndRecv_i4
  
  
  ! *


  subroutine GO_Comm_SendAndRecv_i4_1d( self, values, source, dest, tag, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/GO_Comm_SendAndRecv_i4_1d'
  ...
  end subroutine GO_Comm_SendAndRecv_i4_1d
  ...
  subroutine GO_Comm_BCast_i( self, rootid, value, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/GO_Comm_BCast_i'
  ...
  end subroutine GO_Comm_BCast_i
  ...
  subroutine GO_Comm_BCast_r4( self, rootid, value, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/GO_Comm_BCast_r4'
...
#ifdef _MPI
    ! send value from root to all other pe's:
    call MPI_BCast( value, 1, MPI_REAL, rootid, self%comm, ierror=status )
    IF_MPI_NOT_OK_RETURN(status=1)
#endif
  ...
  end subroutine GO_Comm_BCast_r4
  ...
  subroutine GO_Comm_GatherV_i_2d( self, send, recv, status, &
                                      nloc )
    ...
    character(len=*), parameter   ::  rname = mname//'/GO_Comm_GatherV_i_2d'
  ...
  end subroutine GO_Comm_GatherV_i_2d
  ...
  subroutine GO_Comm_GatherV_s_1d( self, send, recv, status, &
                                      nloc )

#ifdef _MPI
    use MPI_F08, only : MPI_DTYPE => MPI_CHARACTER
    use MPI_F08, only : MPI_GatherV
#endif
    ...
    class(TgoComm), intent(in)          ::  self
    character(len=*), intent(in)        ::  send(:)   ! (max(1,nloc))
    character(len=*), intent(out)       ::  recv(:)   ! (sum nloc)
    integer, intent(out)                ::  status
    ...
    character(len=*), parameter   ::  rname = mname//'/GO_Comm_GatherV_char_32_1d'
    ...
    if ( self%root ) then
      ! check ...
      if ( any( (/len(recv(1)),size(recv)/) /= (/m,ntot/) ) ) then
        write (gol,'("receive buffer has shape (",i0,",",i0,") while (m,ntot) is (",i0,",",i0,")")') &
                        len(recv(1)),size(recv), m,ntot; call goErr
        TRACEBACK; status=1; return
      end if
    end if
  ...
  end subroutine GO_Comm_GatherV_s_1d
  ...
  subroutine GO_Comm_ParInfo( self, n, status, &
                                ntot, istart, recvcounts, displs, displ )
