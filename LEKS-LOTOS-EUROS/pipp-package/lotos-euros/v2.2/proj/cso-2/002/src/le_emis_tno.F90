...
module LE_Emis_TNO
  ...
  use LE_Emis_TNO_Timeprof_Selection, only : T_Emis_Time_Prof_Selection
  use LE_Emis_Timeprof_Hourly, only : T_Emis_Timeprof_Hourly
  use LE_Emis_Timeprof_Hourly_Gridded, only : T_Emis_Timeprof_Hourly_Gridded
  ...
  type T_Emis_TNO
    ...
    type(T_Emis_Time_Prof_Grid)       ::  time_prof_grid(max_time_type)
    
    logical                                            ::  use_timeprof_selection
    type(T_Emis_Time_Prof_Selection)                   ::  time_prof_selection
    type(T_Emis_Timeprof_Hourly), allocatable          ::  tprof_hourly(:)
    type(T_Emis_Timeprof_Hourly_Gridded), allocatable  ::  tprof_hourly_gridded(:)
    
    character(len=512)                                 ::  indir
    character(len=64), allocatable                     ::  fname_hourly(:)
    character(len=64), allocatable                     ::  fname_hourly_gridded(:)
...
contains
  ...
  subroutine LE_Emis_TNO_Read( self, year, status )
    ...
    use LE_Emis_TNO_Timeprof_Selection , only : LE_Emis_TNO_Timeprof_Selection_Init
    use LE_Emis_Timeprof_Hourly        , only : LE_Emis_Timeprof_Hourly_Init
    use LE_Emis_Timeprof_Hourly_gridded, only : LE_Emis_Timeprof_Hourly_Gridded_Init
    ...
    do iemis = 1, self%nemis
      ...
      select case ( trim(self%emis_name(iemis)) )
        ...
        case ( 'co2', 'co2_ff', 'co2_bf' ) ; self%xm(iemis) = xm_C + xm_O * 2  ! kg/mole
    ...
    !--------------------------------------------------------
    write (gol,'("   Read time profile selections ...")') ; call goPr
    !--------------------------------------------------------
    
    ! Read selection table
    call ReadRc( rcF, trim(self%rckey)//'.time_profile_selection', query, status )
    IF_NOTOK_RETURN(status=1)
    
    if ( len(trim(query)) > 0 ) then

      ! set flag
      self%use_timeprof_selection = .true.

      ! Fill
      call LE_Emis_TNO_Timeprof_Selection_Init( self%time_prof_selection, self%cat_code, self%emis_name(1:self%nemis), query, self%year, status )
      IF_NOTOK_RETURN(status=1)

      ! Read necessary time profiles:
      !
      ! *** Type: Hourly --> read direct profile for full year
      !         : hourly-gridded --> only initialize to get file id's
      !                              read gridded profile at start of each hour for full grid

      write( gol, '("Allocate time arrays")') ; call goPr

      ! Initialize
      self%ntype_hourly = self%time_prof_selection%ntype_hourly
      self%ntype_hourly_gridded = self%time_prof_selection%ntype_hourly_gridded

      allocate( self%tprof_hourly(self%ntype_hourly), stat=status )
      IF_NOTOK_RETURN(status=1)
      allocate( self%tprof_hourly_gridded(self%ntype_hourly_gridded), stat=status )
      IF_NOTOK_RETURN(status=1)


      allocate( self%fname_hourly(self%ntype_hourly), stat=status )
      IF_NOTOK_RETURN(status=1)
      allocate( self%fname_hourly_gridded(self%ntype_hourly_gridded), stat=status )
      IF_NOTOK_RETURN(status=1)

      ! fill
      self%fname_hourly(1:self%ntype_hourly) = self%time_prof_selection%fname_hourly(1:self%ntype_hourly)
      self%fname_hourly_gridded(1:self%ntype_hourly_gridded) = &
        self%time_prof_selection%fname_hourly_gridded(1:self%ntype_hourly_gridded)

      ! Init (and Read)s hourly profiles
      call ReadRC( rcF, trim(self%rckey)//'.time_profiles_indir', self%indir, status )
      IF_NOTOK_RETURN(status=1)

      do ityp = 1, self%ntype_hourly
        call LE_Emis_Timeprof_Hourly_Init( self%tprof_hourly(ityp), trim(self%indir), self%fname_hourly(ityp), status )
        IF_NOTOK_RETURN(status=1)
      end do

      ! Initialize hourly gridded profiles
      do ityp = 1, self%ntype_hourly_gridded
        call LE_Emis_Timeprof_Hourly_Gridded_Init( self%tprof_hourly_gridded(ityp), trim(self%indir), self%fname_hourly_gridded(ityp), status )
        IF_NOTOK_RETURN(status=1)
      end do
    
    else 
      ! set flag
      self%use_timeprof_selection = .false.
    
    end if  ! timeprofile selections?
    ...
    do iquery = 1, self%ntimetype
      ...
      ! special value or file specification ?
        ...
        !--------------------------------------------------------
          ...
          ! read:
          call LE_Emis_Time_Prof_Grid_Init( self%time_prof_grid(iquery), query, &
    ...
    if ( .not. exist ) then
      ...
      self%temp_var_VOC = 1.0
      self%temp_var_CO  = 1.0
  ...
  subroutine LE_Emis_TNO_Clear( self, status )
    ...
    do itimetyp = 1, self%ntimetype
      call LE_Emis_Time_Prof_Grid_Done(self%time_prof_grid(itimetyp), status)
      IF_NOTOK_RETURN(status=1)
    end do
    ...
    if ( allocated( self%tprof_hourly) ) then
      deallocate( self%tprof_hourly, stat=status )
      IF_NOTOK_RETURN(status=1)
    end if
    if ( allocated( self%tprof_hourly_gridded) ) then
      deallocate( self%tprof_hourly_gridded, stat=status )
      IF_NOTOK_RETURN(status=1)
    end if
    if ( allocated( self%fname_hourly) ) then
      deallocate( self%fname_hourly, stat=status )
      IF_NOTOK_RETURN(status=1)
    end if
    if ( allocated( self%fname_hourly_gridded) ) then
      deallocate( self%fname_hourly_gridded, stat=status )
      IF_NOTOK_RETURN(status=1)
    end if
  ...
  subroutine LE_Emis_TNO_Setup( self, emis_a, t1, t2, status )
    ...
    use Binas    , only : Avog
    use Num      , only : IntervalSum, Interval
...
#endif
    use LE_Emis_Timeprof_Hourly_gridded, only : LE_Emis_Timeprof_Hourly_Gridded_Setup
    ...
    integer               ::  iicat
    integer               ::  itimetyp    
    character(len=32)     ::  tprof_type
    character(len=512)    ::  tprof_fname
    integer               ::  iprof
    ...
    ! Read current field values for hourly gridded timeprofiles 
    julian_day = calc_DayNumber('gregorian',yy,mm,dd)
    ihour = (julian_day-1)*24+hh + 1
    do itype = 1, self%ntype_hourly_gridded
      call LE_Emis_Timeprof_Hourly_Gridded_Setup( self%tprof_hourly_gridded(itype), ihour, status )
      IF_NOTOK_RETURN(status=1)
    end do
    ...
    ! dummy storage to avoid warnings:
    allocate( hd_profiles(1,1,1,1,1), stat=status )
    IF_NOTOK_RETURN(status=1)
    ...
    do icat = 1, self%emb%ncat
      ...
      do ityp = 1, self%emb%ntyp
        ...
        do isource = 1, self%emb%emg(icat,ityp)%nsource
          ...
          height   = self%emb%emg(icat,ityp)%h(isource)
          ...
          do itimetyp = 1, self%ntimetype
            ...
            select case ( trim(self%timetype(itimetyp) ) )
              ...
              case ( 'hourly-gridded' )
                ...
                do iicat = 1, self%time_prof_grid(itimetyp)%ncat 
                  ! match icat with categories in grid dependent file
                  if ( trim(self%time_prof_grid(itimetyp)%icats(iicat)) == trim(self%emb%cati(icat)%code) ) then                    
                    ! set the time factor (component dependent)
                    julian_day = calc_DayNumber('gregorian',yy,mm,dd)
                    ihour = (julian_day-1)*24+hh + 1
                    ! fill in timefactor from grid dependent fileS
                    time_fact(:) = self%time_prof_grid(itimetyp)%profile(ihour,iicat,ix,iy)
                    exit
                  end if
                end do
              !~ strange ...
...
!          end if
          ...
          ! Put emission at source height
          if ( height > 0 ) then
              ! storage for model layer heighs:
              hhb(0)    = 0.0  ! m
              hhb(1:nz) = h_m(ix,iy,1:nz)   ! m
              
              call Interval( hhb, height, iiz, status )
              IF_ERROR_RETURN(status=1)
          ...
          if ( associated(self%temp_var_CO) .or. associated(self%temp_var_VOC) ) then
            ...
            if (temp_fac_VOC < 0.0) then
               write( gol, '("trying to use an uninitialized temperature factor for VOC", f10.4)' ) temp_fac_VOC ; call GoErr
               write( gol, '("Category index: ", i0, "Temperature index: ", i0)' ) icat,temp_index; call GoErr
               TRACEBACK;status=1;return
            endif
            if (temp_fac_CO < 0.0) then
               write( gol, '("trying to use an uninitialized temperature factor for CO", f10.4)' ) temp_fac_CO ; call GoErr
               write( gol, '("Category index: ", i0, "Temperature index: ", i0)' ) icat,temp_index; call GoErr
               TRACEBACK;status=1;return
            endif
          else
          ...
          do iemis = 1, self%nemis
            ...
            if ( self%use_timeprof_selection ) then
              ! time profile
              tprof_type = self%time_prof_selection%tprof_type(icat,iemis)
              tprof_fname = self%time_prof_selection%tprof_fname(icat,iemis)

              if ( trim(tprof_type) == 'hourly' ) then
                ! select index
                call GoMatchValue( trim(tprof_fname), self%fname_hourly, iprof, status )
                IF_NOTOK_RETURN(status=1)
                
                julian_day = calc_DayNumber('gregorian',yy,mm,dd)
                ihour = (julian_day-1)*24+hhh + 1
                time_fact(iemis) = self%tprof_hourly(iprof)%factor(ihour)

              else if ( trim(tprof_type) == 'hourly-gridded') then  
                ! select index
                call GoMatchValue( trim(tprof_fname), self%fname_hourly_gridded, iprof, status )
                IF_NOTOK_RETURN(status=1)
                time_fact(iemis) = self%tprof_hourly_gridded(iprof)%factor(ix,iy)

              else
                write( gol, '("Unknown timeprofile type found: ", a)' ) trim(tprof_type) ; call GoErr
                TRACEBACK;status=1;return

              end if
            end if  ! Use timeprofile selections?
                        
            ! check ...
            if ( time_fact(iemis) < 0.0 ) then
              write( gol, '("Found negative time factor : ",e16.8)') time_fact(iemis); call goErr
              write( gol, '("Tracer: ",a)' ) trim(self%emis_name(iemis)) ; call goErr
              write( gol, '("Category: ",a )' ) trim(self%emb%cati(icat)%code) ; call goErr
              write( gol, '("Country: ",a)' ) trim(self%emb%cnti(icountry)%code) ; call goErr
              TRACEBACK; status=1; return
            end if
                ...
                !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                  ...
                  do icomp = 1, self%emcomp(iemis)%ncomp
                    ...
                    if ( compfrac < 0.0 ) then
                      ...
                      write (gol,'("  category   : ",a)') trim(self%cat_code(icat)); call goErr
                      write (gol,'("  country    : ",a)') trim(self%country_code(icountry)); call goErr
                      write (gol,'("  component  : ",a)') trim(specname(ispec)); call goErr
                      write (gol,'(" Emission amount: ",e12.4)' ) emis(iemis); call goErr
                      write (gol,'(" Indices    : ", i0," ",i0)' ) ix,iy ; call goErr
                      write (gol,'(" Compfrac    : ", e12.4)' ) compfrac ; call goErr
                      TRACEBACK; status_par=status_par+1; cycle
                    end if
                    ! units of emissions and target tracer:
                    conversion = trim(self%emb%emi(iemis)%units)//' -> '//trim(specunit(ispec))
              ...
              !=================================================
                ...
                do icomp = 1, self%emcomp(iemis)%ncomp
                  ...
                  if ( compfrac < 0.0 ) then
                    write (gol,'("found undefined component fraction for emission:")'); call goErr
                    write (gol,'("  emission   : ",a)') trim(self%emis_name(iemis)); call goErr
                    write (gol,'("  category   : ",a)') self%cat_code(icat); call goErr
