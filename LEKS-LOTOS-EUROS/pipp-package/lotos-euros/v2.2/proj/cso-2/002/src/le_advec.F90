...
contains
  ...
  subroutine LE_Advec_Init( status )
    ...
    ! --- begin ----------------------------------
    !AYB added uv_u_met
    call LE_Data_Enable( 'uv_u_met',status )        
    IF_NOTOK_RETURN(status=1)
    call LE_Data_Enable( 'uv_v_met',status )        
    IF_NOTOK_RETURN(status=1)
    call LE_Data_Enable( 'kh',status )        
    IF_NOTOK_RETURN(status=1)
    !! define timers:
  ...
  subroutine LE_Advec_Get_NStep( dt_min, nstep, status )
...
#ifdef with_hdiff
    use LE_HDiff     , only : hordif, difcoef
#endif
    ...
    call LE_Data_GetPointer( 'kh'     , kh    , status, check_units='1/s' )
    IF_NOTOK_RETURN(status=1)
    
    ! Exchange boundaries
    allocate(kh_loc(0:nx+1,0:ny+1,1:nz), stat=status )
    IF_NOTOK_RETURN(status=1)

    kh_loc = 0.0    
    kh_loc(1:nx,1:ny,1:nz) = kh
    call dom%ExchangeHaloX( 1, kh_loc, status )
    IF_NOTOK_RETURN(status=1)
    call dom%ExchangeHaloY( 1, kh_loc, status )
    IF_NOTOK_RETURN(status=1)    

    ! calculate diffusion coefficients
    call difcoef( kh_loc(0:nx+1,0:ny+1,1:nz), status )
    IF_NOTOK_RETURN(status=1)
    
    ! Courant number for diffusion      
    call courant_d( nx,ny,nz, inv_volume, dt_min, dtd, nstep_d, status )
    IF_NOTOK_RETURN(status=1)
    
    ! clear:
    deallocate( kh_loc, stat=status )
    IF_NOTOK_RETURN(status=1)
    
    ! check whether diffusion is dominating over advection
    nstep = max( nstep, nstep_d )
#endif
    ...
    call goc_dom%AllReduce( 'max', nstep, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine LE_Advec_Apply( c, dt, status )
    ...
    use SA_Labeling, only : SA_Hdiff_Setup
    use SA_Labeling, only : SA_frac_hdiff
#endif
    use LE_Data      , only : LE_Data_GetPointer
...
!        integer       ::  off(2)
    ...
    call courant_d( nx,ny,nz, inv_volume, dt, dtd, nstep_d, status )
    IF_NOTOK_RETURN(status=1)
    
    ! check whether diffusion or advection is dominating
    nstep_a = max( nstep_a, nstep_d )
    dta     = min( dta    ,dtd  )
    ...
    call goc_dom%AllReduce( 'min', dta, status )
    IF_NOTOK_RETURN(status=1)
    call goc_dom%AllReduce( 'max', nstep_a, status )
    IF_NOTOK_RETURN(status=1)
!#ifdef with_hdiff
!    call goc_dom%AllReduce( 'min', dtd, status )
!    IF_NOTOK_RETURN(status=1)
!    call goc_dom%AllReduce( 'max', nstep_d, status )
!    IF_NOTOK_RETURN(status=1)
!    !if (nstep_d > nstep_a .AND. (.NOT.outF%suppress) ) then
!    !  write (*,*) ident2, '  >>> diffusion dominates advection'
!    !write (*,*) ident2, '  >>> number of steps required for hor diffusion: ', nstep_d
...
!#endif    
    ...
    allocate( dens0(1,1,1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( dens1(1,1,1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( inv_d2(1,1,1), stat=status )
    IF_NOTOK_RETURN(status=1)
    ! clear dummy storage:
    ...
    if (status/=0) status_par = status_par + 1
    allocate( dummy4_hd(0:nx+1,0:ny+1,1:nz,1:SA_nlabel), stat=status )
    if (status/=0) status_par = status_par + 1
#endif
    ...
    do itr = 1, n_advected
      ...
      if ( SA_ispec > 0 ) then
        ! Exchange fractions from neighbour grids
        do SA_ilabel = 1, SA_nlabel
          dummy4_hd(:,:,:,SA_ilabel) = SA_frac_hdiff(SA_ilabel,:,:,:,SA_ispec)
        end do
        call dom%ExchangeHaloX( 1, dummy4_hd, status )
        if ( status /= 0 ) then
          write (gol, '(" Error from exchange of labelfractions halo cells x-direction")' ); call goErr
          status_par = status_par + 1
        end if
        call dom%ExchangeHaloY( 1, dummy4_hd, status )
        if ( status /= 0 ) then
          write (gol, '(" Error from exchange of labelfractions halo cells y-direction")' ); call goErr
          status_par = status_par + 1
        end if
        do SA_ilabel = 1, SA_nlabel
          SA_frac_hdiff(SA_ilabel,:,:,:,SA_ispec) = dummy4_hd(:,:,:,SA_ilabel)
        end do
      end if
#endif
      ...
      do istep = 1, nstep_a
        ! Exchange halo in both x/y direction
        call dom%ExchangeHaloY( nh, ch, status )
        IF_NOTOK_RETURN(status=1) 
        call dom%ExchangeHaloX( nh, ch, status )
        IF_NOTOK_RETURN(status=1) 
        ! do horizontal diffusion for the compound
        call hordif( ch(1:nx,1:ny,1:nz), dtd, &
                     ch(0   ,1:ny,1:nz), ch(nx+1,1:ny,1:nz), &
                     ch(1:nx,0   ,1:nz), ch(1:nx,ny+1,1:nz), &
                     k, status )
        IF_NOTOK_RETURN(status=1)                     
...
#ifdef with_labeling
    deallocate( dummy4_hd, stat=status )
    if (status/=0) status_par = status_par + 1
#endif
...
#ifdef with_hdiff
  subroutine courant_d( nx, ny,nz, inv_volume, dt, dtd, nstep_d, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/courant_d'
    ! --- local ----------------------------------
    ...
    do j=1,ny
    do k=1,nz
       cour = abs( khx(i,j,k)*dt)/dx(i,j,1)/dh(i,j,k)
    ...
    do j=1,ny
    do k=1,nz
       cour = abs( khy(i,j,k)*dt)/dy(i,j,1)/dh(i,j,k)
       cour_max = max(cour_max, cour)
    enddo
    enddo
    enddo
