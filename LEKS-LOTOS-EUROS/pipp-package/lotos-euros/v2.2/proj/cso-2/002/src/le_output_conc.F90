...
!#endif
    ...
    allocate( hh(1,1,1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( hsamp(1,1), stat=status )
    IF_NOTOK_RETURN(status=1)
    ...
    if ( (trim(leo%levtype) == 'heights') .or. &
      ...
      IF_NOTOK_RETURN(status=1)
      ! half level heights:
      allocate( hh(1-leo%nhx:nx+leo%nhx,1-leo%nhy:ny+leo%nhy,0:nlev_top), stat=status )
      IF_NOTOK_RETURN(status=1)
      ! lowest is orography:
      ...
      ! clear dummy storage:
      deallocate( hsamp, stat=status )
      IF_NOTOK_RETURN(status=1)
      ! sample height:
      allocate( hsamp(1-leo%nhx:nx+leo%nhx,1-leo%nhy:ny+leo%nhy), stat=status )
      IF_NOTOK_RETURN(status=1)
    end if
      ...
      do ilev = 0, nlev_top
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          do la = 1, accum_n(itr)
            ! conversion needed ?
            convfact = 1.0
            if ( accum_ppb_to_ugm3(itr,la) ) convfact = dens(1:nx,1:ny,1)/xm_air  ! (mole air)/m3
            ! core:
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          do la = 1, accum_n(itr)
            ! conversion needed ?
            convfact = 1.0
            if ( accum_ppb_to_ugm3(itr,la) ) convfact = dens(1:nx,1:ny,ilev)/xm_air
            ! core:
            pat(leo%i1:leo%i2,leo%j1:leo%j2) = pat(leo%i1:leo%i2,leo%j1:leo%j2) &
