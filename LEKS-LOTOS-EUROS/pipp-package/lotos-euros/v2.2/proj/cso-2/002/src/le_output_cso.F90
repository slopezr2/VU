...
contains
  ...
  subroutine LE_Output_CSO_Data_Init( self, rcF, rcbase, typ, name, status )
    ...
    call self%csod%Init( status, comm=goc_dom%comm )
    IF_NOT_OK_RETURN(status=1)
#else
...
!    integer                   ::  nw
    ...
    if ( len_trim(self%orbit_filename) > 0 ) then
      ...
      call self%sdata%SetupExchange( status )
      IF_NOT_OK_RETURN(status=1)
...
!#endif
      ...
      do iuvar = 1, self%nuvar
        ...
        select case ( trim(self%uvarnames(iuvar)) )
          ...
          case ( 'mod_aod_440' )
            ...
            call self%sstate%GetData( status, name=self%uvarnames(iuvar), data1=data1 )
            IF_NOT_OK_RETURN(status=1)
            write (gol,*) 'test uvarnames',trim(self%uvarnames(iuvar)); call goPr
            write (gol,*) 'test tracer_name',trim(sdat%tracer_name); call goPr
            ! loop over pixels:
            vlen = len_trim(sdat%tracer_name)
            read (sdat%tracer_name(5:vlen-2),*) lambda
            ! band index:
      ...
      call self%sstate%Exchange( sdat%sdata, status )
      IF_NOT_OK_RETURN(status=1)
