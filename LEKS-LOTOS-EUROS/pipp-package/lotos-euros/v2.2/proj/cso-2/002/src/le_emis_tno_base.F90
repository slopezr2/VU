...
!  public  ::  N_SOURCETYPE, SOURCETYPE_LABEL, SOURCETYPE_AREA, SOURCETYPE_POINT
  ...
  ! --- local --------------------------------
  type  T_Grid_Mapping
    integer         ::  i1,i2,j1,j2  ! boundary indices
    real, pointer   ::  ff(:,:)      ! fractions
  end type
  ...
  type T_Emis_Gathered_All
    ...
    ! Source heights
    real, allocatable                       ::  heights(:)  ! (nsource)
    ...
    ! grid mapping
    integer                                 ::  i1,i2,j1,j2
    type(T_Grid_Mapping), allocatable       ::  gmp(:,:)  ! (i1:i2,j1:j2)
...
contains
  ...
  subroutine Emis_Gathered_All_Init( ema, filename, status )
    ...
    character(len=32)       ::  dimname_emis_cat
    character(len=32)       ::  dimname_emis_cat_code_len
    character(len=32)       ::  dimname_emis_cat_name_len
    ...
    character(len=32)       ::  varname_emis_cat_index
    ...
    select case ( trim(format_id) )
      ...
      case ( '1.2' )
        ...
        dimname_emis_cat           = 'emis_cat'
        dimname_emis_cat_name_len  = 'emis_cat_name'
        dimname_emis_cat_code_len  = ''
        ...
        varname_emis_cat_index     = 'emission_category_index'
      ...
      case ( '1.3', '1.4' )
        ...
        dimname_emis_cat           = 'emis_cat'
        dimname_emis_cat_name_len  = 'emis_cat_name_len'
        dimname_emis_cat_code_len  = ''
      ...
      case ( '1.6' )
        ...
        dimname_emis_cat           = 'emis_cat'
        dimname_emis_cat_name_len  = 'emis_cat_name_len'
        dimname_emis_cat_code_len  = 'emis_cat_code_len'
        varname_emis_cat_name_len  = 'emis_cat_name_len'
        varname_emis_cat_name      = 'emis_cat_name'
        ...
        varname_latitude_source    = 'latitude_source'
        attname_units              = 'units'
      !~ latest output of emission model (including fuel information
      case ( '2.0' )
        varname_longitude_bounds   = 'longitude_bounds'
        varname_latitude_bounds    = 'latitude_bounds'
        varname_country_name_len   = 'country_name_len'
        dimname_emis_cat           = 'emis_sector'
        dimname_emis_cat_code_len  = 'emis_sector_code_len'
        dimname_emis_cat_name_len  = 'emis_sector_name_len'
        varname_emis_cat_name      = 'sector_name'
        varname_emis_cat_code      = 'sectorID'
        varname_emis_cat_shortsnap = ''
        varname_emis_cat_index     = 'emission_sector_index'
        varname_longitude_source   = 'longitude_source'
        varname_latitude_source    = 'latitude_source'
        attname_units              = 'units'
      !~ unknown ...
    ...
    ! fill grid:
    allocate( ema%lons(ema%nlon) )
    allocate( ema%lats(ema%nlat) )
    call MDF_Inq_VarID( hid, 'longitude', varid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Get_Var( hid, varid, ema%lons, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Inq_VarID( hid, 'latitude', varid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Get_Var( hid, varid, ema%lats, status )
    IF_NOTOK_RETURN(status=1)
    
    write( gol, '("init grid") ' ) ; call GoPr
        
    ! initialize full grid of emission file
    call grid_in%Init( ema%lons, ema%lonbs, ema%lats, ema%latbs, status )
    IF_NOTOK_RETURN(status=1)
    
    ! Boundaries of model grid
    west  = minval(ugg%longitude_bnds)
    east  = maxval(ugg%longitude_bnds)
    south = minval(ugg%latitude_bnds)
    north = maxval(ugg%latitude_bnds)
    
    ! Find corner indices
    call grid_in%GetLocation( west,south,ema%i1,ema%j1, status )
    IF_NOTOK_RETURN(status=1)
    call grid_in%GetLocation( east,north,ema%i2,ema%j2, status )
    IF_NOTOK_RETURN(status=1)
    
    write( gol, '("Calculate distributions")' ) ; call GoPr
    
    ! grid mapping
    allocate( ema%gmp(ema%i1:ema%i2,ema%j1:ema%j2), stat=status)
    IF_NOTOK_RETURN(status=1)
    ! find regridding factors
    do i = ema%i1,ema%i2
    do j = ema%j1,ema%j2
      ! clear fraction pointer
      nullify ( ema%gmp(i,j)%ff )
      ! Calculate emission distribution to target grid
      call ugg%GetDistribution( ema%lonbs(1,i), ema%lonbs(2,i), ema%latbs(1,j), ema%latbs(2,j), &
                                ema%gmp(i,j)%i1,ema%gmp(i,j)%i2,ema%gmp(i,j)%j1,ema%gmp(i,j)%j2, &
                                  ema%gmp(i,j)%ff, status )
      IF_NOTOK_RETURN(status=1)
    end do ! i
    end do ! j
    ...
    call MDF_Inq_DimID( hid, trim(dimname_emis_cat), dimid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Inquire_Dimension( hid, dimid, status, length=ema%ncat )
    IF_NOTOK_RETURN(status=1)
    ! maximum length of name:
    call MDF_Inq_DimID( hid, trim(dimname_emis_cat_name_len), dimid, status )
    IF_NOTOK_RETURN(status=1)
    ...
    !  
      ...
      ! maximum length of name:
      call MDF_Inq_DimID( hid, trim(dimname_emis_cat_code_len), dimid, status )
      IF_NOTOK_RETURN(status=1)
    ...
    allocate( ema%heights(ema%nsource) )
    
    ! indices in categories array:
    call MDF_Inq_VarID( hid, trim(varname_emis_cat_index), varid , status)
    IF_NOTOK_RETURN(status=1)
    ...
    ! Emission heights (used for point sources, set to dummy value if not available)
    call MDF_Inq_VarID( hid, 'source_height', varid, status )
    if ( status /= 0 ) then
      ema%heights = -999.
    else 
      call MDF_Get_Var( hid, varid, ema%heights, status )
      IF_NOTOK_RETURN(status=1)
    endif
    !
    ! * read emissions
    ...
    do iemis = 1, ema%nemis
      select case ( trim(ema%name(iemis)) )
        case ( 'PPM10', 'pm10', 'pmco' )
          iemis_pm10 = iemis
          exit
        case ( 'ch4', 'co', 'co2','co2_ff','co2_bf','n2o', 'nh3', 'nmvoc', 'nox', 'pm25', 'pm2_5', 'so2', &
               'pm1', 'bc_1', 'ec_1-25', 'ec_25-10', 'oc_25', 'oc_25-10', &
               'ec_coarse', 'ec_fine', 'oc_coarse', 'oc_fine', 'so4_coarse', 'so4_fine', &
               'na_coarse', 'na_fine', 'othmin_coarse', 'othmin_fine', &
               'so4ks', 'so4as', &
  ...
  subroutine Emis_Gathered_All_Done( ema, status )
    ...
    do i = ema%i1, ema%i2
    do j = ema%j1, ema%j2
      deallocate( ema%gmp(i,j)%ff, stat=status )
      IF_NOTOK_RETURN(status=1)
    end do
    end do
    deallocate( ema%gmp, stat=status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine Emis_Gathered_Init( emg, ema, icat, ityp, status )
    ...
    allocate( indx(nx,ny,emg%ncountry), stat=status )
    IF_NOTOK_RETURN(status=1)
    ...
    do iloop = 1, 2
      ...
      do isrc = 1, ema%nsource
        ...
        ilat = ema%ilats(isrc)
        icnt = ema%icnts(isrc)
        height = ema%heights(isrc)
        
        ! Check if in target domain
        if ( ilon < ema%i1 .or. ilon > ema%i2 .or. ilat < ema%j1 .or. ilat > ema%j2 ) cycle
        ...
        select case ( ema%typs(ityp:ityp) )
          ...
          !~~~~~~~~~~~~~~~
            ...
            i1 = ema%gmp(ilon,ilat)%i1
            i2 = ema%gmp(ilon,ilat)%i2
            j1 = ema%gmp(ilon,ilat)%j1
            j2 = ema%gmp(ilon,ilat)%j2
            
            nullify( ff )
            ff => ema%gmp(ilon,ilat)%ff
          ...
          !~~~~~~~~~~~~~~~
            ...
            lat = ema%plats(isrc)
            ! grid cell:
            call ugg%GetLocation( lon, lat, i1, j1, status, quiet=.true. )
      ...
      if ( iloop == 1 ) then
        ...
        if ( n > 0 ) then
          ! info ...
          write (gol,'("    gathered sources for cat ",i3," typ ",i1," : ",i8)') icat, ityp, n; call goPr
    ...
    !  deallocate( ff, stat=status )
    !  IF_NOTOK_RETURN(status=1)
    !end if
    !deallocate( indx, stat=status )
    !IF_NOTOK_RETURN(status=1)
