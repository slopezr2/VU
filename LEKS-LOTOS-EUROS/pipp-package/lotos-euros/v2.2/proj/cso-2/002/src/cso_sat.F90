...
!Santiago add read name_error and error variable from CSO file
  subroutine CSO_Sat_Data_GetPixel( self, ipix, status, &
                                            ...
                                            name, name_error, name_cov,value, profile, obs_error,r_loc, &
    ...
    character(len=*), intent(in), optional        ::  name_cov ! Name of cov variable
    real, pointer, optional                       ::  value
    real, pointer, optional                       ::  profile(:)
    real, pointer, optional                       ::  obs_error(:)
    real, pointer, optional                       ::  r_loc(:,:)
    ...
    ! return covariance ?
    if ( present(r_loc) ) then
      ! get pointer:
      call self%pd%GetData( status, name=name_cov, data2=data2 )
      IF_NOT_OK_RETURN(status=1)
      ! assign:
      r_loc => data2(:,:,ipix)
    end if
