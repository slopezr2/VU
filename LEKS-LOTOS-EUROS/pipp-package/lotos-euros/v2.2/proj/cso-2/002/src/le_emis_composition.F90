...
contains
        ...
        ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          do ifield = 1, nheader
            ...
            select case ( trim(header) )
              ...
              !~ sector used in sector/fuel combined emissions (GNFR based)
              case ( 'Sector_Fuel' )
                ifield_sector = ifield
                ...
                select case ( trim(emitted_tracer) )
                  ...
                  case ( 'ppm_f', 'pm2_5', 'pm25' )
                    ...
                    select case ( trim(header) )
                      ...
                      case ( 'Fraction_Na'                      , 'Na_fine'     ) ; component = 'na_f2'
                  ...
                  case ( 'ppm_c', 'pm25_pm10' )
                    ...
                    select case ( trim(header) )
                      ...
                      case ( 'Fraction_Na'                      , 'Na_coarse'     ) ; component = 'na_c3'
          ...
          ! check index:
          if ( ifield_cat < 0 .and. ifield_sectorid < 0 .and. ifield_sector < 0) then
          ...
          do
            ...
            ! check year (skip other years)
            if ( ifield_year > 0 ) then
              read( fields(ifield_year), * ) year_rd
              if ( .not. year_rd == year ) cycle
            end if
            ...
            if ( ifield_sector > 0 ) then
              ! search ...
              ! sector
              call goMatchValue( trim(fields(ifield_sector)), cat_codes, icat, status )
              IF_NOTOK_RETURN(status=1)              

            else if ( ifield_sectorid > 0 ) then
            ...
            do ifield = 1, nfield
              ...
              field = trim(fields(ifield))
              
              ! read fraction:
              read (field,*,iostat=status) frac
              if (status/=0) then
                write (gol,'("reading fraction from `",a,"`")') trim(field); call goErr
                write (gol,'("Composition file: `", a,"`")' ) trim(filename) ; call goErr
                write (gol,'("Line: `",i8,"`")' ) iline ; call goErr
                TRACEBACK; status=1; return
              end if
  ...
  subroutine EmisComp_MassSplit_Init( self, filename, cat_codes, country_codes, year, status )
    ...
    write( gol, '("Read NMVOC composition mass split from: ",a, " for year: ", i0)' ) trim(filename), year ; call goPr
    ...
    if (status/=0) then
      write (gol,'("opening file : ",a)') trim(filename); call goErr
      write (gol,'("status: ",i0)' ) status  ; call goErr
      TRACEBACK; status=1; return
    end if
    ...
    do ifield = 1, nheader
      ...
      select case ( trim(header) )
        ...
        case ( 'ISO3', 'country', 'Country' )
          ...
          ifield_country = ifield
        !~ category code:
        case ( 'GNFR_Category', 'SectorID', 'Sector_Fuel' )
          ! store field index:
          ifield_cat = ifield
        !~ substance ...
        case default
    ...
    do
      ...
      if (status/=0) then
        ...
        write (gol,'("ifield_year: ", i0)' ) ifield_year; call goErr
        TRACEBACK; status=1; return
      end if
      ! filter ...
      ...
      ! get country code:
      call goMatchValue( fields(ifield_country), country_codes, icountry, status, quiet=.true. )
      IF_ERROR_RETURN(status=1)
...
!        do icountry = 1, self%ncountry
!          if ( any( self%fraction(icat,icountry,:) < 0.0 ) ) then
!            !self%fraction(icat,icountry,:) = 0.0
...
!    end if
  ...
  subroutine EmisComp_CarbonBonds_Done( self, status )
    ...
    IF_NOTOK_RETURN(status=1)
    deallocate( self%bond_code, stat=status )
    IF_NOTOK_RETURN(status=1)
