...
module LE_Driver
  ...
  ! optional: input of concentrations
  type(T_LE_Input)        ::  lei
...
contains
  ...
  subroutine LE_Model_Init( t, dt, nhour, status )
    ...
    IF_NOTOK_RETURN(status=1)
    call GO_Timer_Def( itim_input   , 'time step input', status )
    IF_NOTOK_RETURN(status=1)
#ifdef with_m7    
    ...
    if ( any( (/nop_chem,nop_vdif,nop_sedim,nop_depos,nop_wetdep,nop_emis,nop_advec/) > 1 ) ) then
      write (gol,'("duplicated operators found in sequence: ",a)') trim(line); call goErr
    ...
    ! flags:
    with_chem   = nop_chem   == 1
    with_vdif   = nop_vdif   == 1
    with_wetdep = nop_wetdep == 1
    with_emis   = nop_emis   == 1
    with_advec  = nop_advec  == 1
    ! adhoc: depos needed for mix2ground ...
    !with_depos  = nop_depos  == 1
    !with_sedim  = nop_sedim  == 1
    with_depos  = .true.
    with_sedim  = .true.
    ...
    if ( with_advec ) then
      ...
      ! info ..
      write (gol,'("WARNING - advection disabled!")'); call goPr
    end if ! with_advec
    ...
    if ( with_vdif ) then
      ...
      ! info ..
      write (gol,'("WARNING - vertical diffusion disabled!")'); call goPr      
    end if ! with_vdif
    ...
    if ( with_chem ) then
      ...
      ! info ..
      write (gol,'("WARNING - chemistry disabled!")'); call goPr      
    end if ! with_vdif
    ...
    if ( with_sedim ) then
      ...
      ! info ..
      write (gol,'("WARNING - sedimentation disabled!")'); call goPr      
    end if ! with_depos
    ...
    if ( with_depos ) then
      ...
      ! info ..
      write (gol,'("WARNING - dry deposition disabled!")'); call goPr      
    end if ! with_depos
    ...
    if ( with_wetdep ) then
      ...
      ! info ..
      write (gol,'("WARNING - wet deposition disabled!")'); call goPr      
    end if ! with_wetdep
    ...
    if ( with_emis ) then
      ...
      ! info ..
      write (gol,'("WARNING - emissions disabled!")'); call goPr      
    end if ! with_emis


    !
    ! ** input of concentrations from previous run
    !

    ! init optional concentration input:
    call lei%Init( rcF, 'le.input', status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine LE_Model_Done( status )
    ...
    ! done with optional concentration input:
    call lei%Done( status )
    IF_NOTOK_RETURN(status=1)
  ...
  ! without_physics
    ...
    ! ensure that first time step flag is reset now,
    ! even in case no operators were applied:
    runF%first = .false.

    ! ~

    ! start timing:
    call GO_Timer_Start(itim_input, status )
    IF_NOTOK_RETURN(status=1)
    ! read concentrations if necessary, this replaces concentrations:
    call lei%PutIn( t+dt, c, status )
    IF_NOTOK_RETURN(status=1)
    ! end timing:
    call GO_Timer_End(itim_input, status )
    IF_NOTOK_RETURN(status=1)
