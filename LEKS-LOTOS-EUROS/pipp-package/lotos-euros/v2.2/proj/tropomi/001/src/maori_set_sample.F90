...
module MAORI_Set_Sample
  ...
  use MAORI_Param, only : MAORI_LEN_NAME, MAORI_LEN_LONGNAME, MAORI_LEN_FILE, MAORI_LEN_LINE
...
contains
  ...
  subroutine MAORI_Set_Output_State_Write( moutput, mstate, mdata, status )
    ...
    integer                                            ::  ivar,ilev,nloc
    type(TDate)                                        ::  t
    integer                                            ::  time6(6)
    real                                               ::  time
    integer                                            ::  ihist
    integer, allocatable                               ::  nlocs(:)
    character(len=MAORI_LEN_LONGNAME), allocatable     ::  loc_stn_name_all(:)
    character(len=MAORI_LEN_NAME), allocatable         ::  loc_stn_code_all(:)
    character(len=MAORI_LEN_LONGNAME), allocatable     ::  loc_stn_name_local(:)
    character(len=MAORI_LEN_NAME), allocatable         ::  loc_stn_code_local(:)
    real, allocatable                                  ::  loc_stn_lon_all(:)
    real, allocatable                                  ::  loc_stn_lat_all(:)
    real, allocatable                                  ::  loc_stn_alt_all(:)
    real, allocatable                                  ::  loc_sample_aver_all(:,:,:)
    real, allocatable                                  ::  loc_obs_value_all(:,:,:)
    real, allocatable                                  ::  loc_obs_errstd_all(:,:,:)
    integer, allocatable                               ::  loc_obs_astat_all(:,:,:)
    integer                                            ::  iloc0
    ...
    !! testing ...
    !write (gol,*) ' moutput%opened', moutput%opened; call goPr
    ! new file ?
    ...
    iloc0 = 0 ! stays zero, clean up code future
    ...
    do ihist = 1, mdata%nhist
      ...
      ! collect all data on root
      if ( goc%root ) then
        ! storage for all data:
        allocate(loc_sample_aver_all(mdata%maxlev,mdata%nvar,nloc), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate(loc_obs_value_all(mdata%obs_nlev,mdata%obs_nvar,nloc), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate(loc_obs_errstd_all(mdata%obs_nlev,mdata%obs_nvar,nloc), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate(loc_obs_astat_all(mdata%obs_nlev,mdata%obs_nvar,nloc), stat=status )
        IF_NOTOK_RETURN(status=1)
      else
        ! dummy ...
        allocate(loc_sample_aver_all(1,1,1), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate(loc_obs_value_all(1,1,1), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate(loc_obs_errstd_all(1,1,1), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate(loc_obs_astat_all(1,1,1), stat=status )
        IF_NOTOK_RETURN(status=1)
      end if ! root
      ...
      if ( moutput%itrec == 1 ) then
        ...
        ! create collections for station info
        allocate( loc_stn_name_local(mdata%nloc), stat=status )
        IF_NOTOK_RETURN(status=1)
        allocate( loc_stn_code_local(mdata%nloc), stat=status )
        IF_NOTOK_RETURN(status=1)
        
        if ( goc%root ) then
          ! storage for all data:
          allocate( loc_stn_name_all(nloc), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_code_all(nloc), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_lon_all(nloc), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_lat_all(nloc), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_alt_all(nloc), stat=status )
          IF_NOTOK_RETURN(status=1)
        else
          ! dummy ...
          allocate( loc_stn_name_all(1), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_code_all(1), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_lon_all(1), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_lat_all(1), stat=status )
          IF_NOTOK_RETURN(status=1)
          allocate( loc_stn_alt_all(1), stat=status )
          IF_NOTOK_RETURN(status=1)
        end if
        ! actual collecting -->loc(1:mdata%nloc)
        loc_stn_name_local(1:mdata%nloc) = mdata%loclist%loc(1:mdata%nloc)%name
        loc_stn_code_local(1:mdata%nloc) = mdata%loclist%loc(1:mdata%nloc)%code
        call goc%GatherV( loc_stn_name_local, loc_stn_name_all, status, nloc=mdata%nloc )
        IF_NOTOK_RETURN(status=1)
        call goc%GatherV( loc_stn_code_local, loc_stn_code_all, status, nloc=mdata%nloc )
        IF_NOTOK_RETURN(status=1)
        call goc%GatherV( mdata%loclist%loc(1:mdata%nloc)%lon, loc_stn_lon_all, status, nloc=mdata%nloc )
        IF_NOTOK_RETURN(status=1)
        call goc%GatherV( mdata%loclist%loc(1:mdata%nloc)%lat, loc_stn_lat_all, status, nloc=mdata%nloc )
        IF_NOTOK_RETURN(status=1)
        call goc%GatherV( mdata%loclist%loc(1:mdata%nloc)%alt, loc_stn_alt_all, status, nloc=mdata%nloc )
        IF_NOTOK_RETURN(status=1)
        ! end collection
        ...
        if ( goc%root ) then
          ...
          status = NF90_Put_Var( moutput%ncid, moutput%varid_stn_lon, &
                       loc_stn_lon_all(1:nloc), &
                       start=(/iloc0+1/), count=(/nloc/) )
          IF_NF90_NOTOK_RETURN(status=1)
          status = NF90_Put_Var( moutput%ncid, moutput%varid_stn_lat, &
                       loc_stn_lat_all(1:nloc), &
                       start=(/iloc0+1/), count=(/nloc/) )
          IF_NF90_NOTOK_RETURN(status=1)
          status = NF90_Put_Var( moutput%ncid, moutput%varid_stn_alt, &
                       loc_stn_alt_all(1:nloc), &
                       start=(/iloc0+1/), count=(/nloc/) )
          IF_NF90_NOTOK_RETURN(status=1)
        end if ! root

        ! clear:
        deallocate( loc_stn_name_all, stat=status )
        IF_NOTOK_RETURN(status=1)
        deallocate( loc_stn_code_all, stat=status )
        IF_NOTOK_RETURN(status=1)
        deallocate( loc_stn_lon_all, stat=status )
        IF_NOTOK_RETURN(status=1)
        deallocate( loc_stn_lat_all, stat=status )
        IF_NOTOK_RETURN(status=1)
        deallocate( loc_stn_alt_all, stat=status )
        IF_NOTOK_RETURN(status=1)
        deallocate( loc_stn_name_local, stat=status )
        IF_NOTOK_RETURN(status=1)
        deallocate( loc_stn_code_local, stat=status )
        IF_NOTOK_RETURN(status=1)
        write (gol,*) 'reached end dealloc'; call goPr
      ...
      do ivar = 1, mdata%nvar
        do ilev = 1, mdata%nlev(ivar)
          call goc%GatherV( mstate%sample_aver(:,ilev,ivar,ihist), loc_sample_aver_all(ilev,ivar,:), status, nloc=mdata%nloc )
          IF_NOTOK_RETURN(status=1)
        end do
      end do
      do ivar = 1, mdata%obs_nvar
        do ilev = 1, mdata%obs_nlev 
          call goc%GatherV( mdata%obs_value(:,ilev,ivar,ihist), loc_obs_value_all(ilev,ivar,:), status, nloc=mdata%nloc )
          IF_NOTOK_RETURN(status=1)
          call goc%GatherV( mdata%obs_errstd(:,ilev,ivar,ihist), loc_obs_errstd_all(ilev,ivar,:), status, nloc=mdata%nloc )
          IF_NOTOK_RETURN(status=1)
          call goc%GatherV( mdata%obs_astat(:,ilev,ivar,ihist), loc_obs_astat_all(ilev,ivar,:), status, nloc=mdata%nloc )
          IF_NOTOK_RETURN(status=1)
        end do
      end do
      ...
      if ( goc%root ) then
        ...
        do ivar = 1, mdata%nvar
          ...
          status = NF90_Put_Var( moutput%ncid, moutput%varid(ivar), &
                         loc_sample_aver_all(1:mdata%nlev(ivar),ivar,1:nloc), &
                         start=(/iloc0+1,1,moutput%itrec/), &
                         count=(/nloc,mdata%nlev(ivar),1/) )
          IF_NF90_NOTOK_RETURN(status=1)
        end do
        ...
        if ( mdata%obs_nvar > 0 ) then
          ...
          do ivar = 1, mdata%obs_nvar
            ...
            status = NF90_Put_Var( moutput%ncid, moutput%varid_y(ivar), &
                           loc_obs_value_all(1:mdata%obs_nlev,ivar,1:nloc), &
                           start=(/iloc0+1,1,moutput%itrec/), &
                           count=(/nloc,mdata%obs_nlev,1/) )
            IF_NF90_NOTOK_RETURN(status=1)
            ! write error standard deviations:
            status = NF90_Put_Var( moutput%ncid, moutput%varid_r(ivar), &
                           loc_obs_errstd_all(1:mdata%obs_nlev,ivar,1:nloc), &
                           start=(/iloc0+1,1,moutput%itrec/), &
                           count=(/nloc,mdata%obs_nlev,1/) )
            IF_NF90_NOTOK_RETURN(status=1)
            ! write assimilation status:
            status = NF90_Put_Var( moutput%ncid, moutput%varid_astat(ivar), &
                           loc_obs_astat_all(1:mdata%obs_nlev,ivar,1:nloc), &
                           start=(/iloc0+1,1,moutput%itrec/), &
                           count=(/nloc,mdata%obs_nlev,1/) )
            IF_NF90_NOTOK_RETURN(status=1)
          end do
      ...
      ! clear
      deallocate(loc_sample_aver_all, stat=status )
      IF_NOTOK_RETURN(status=1)
      deallocate(loc_obs_value_all, stat=status )
      IF_NOTOK_RETURN(status=1)
      deallocate(loc_obs_errstd_all, stat=status )
      IF_NOTOK_RETURN(status=1)
      deallocate(loc_obs_astat_all, stat=status )
      IF_NOTOK_RETURN(status=1)
