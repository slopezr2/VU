...
contains
  ...
  subroutine AerosolModes_Init( self, status )
    ...
    do i_aerosol = 1, n_aerosol
      ...
      ! check ...
      !write (*,*) 'Santiago R_g ' ,trim(specname(ispec)),' : ', R_g
      if ( R_g < 0.0 ) then
             write (gol,'("no aerosol radius defined for aerosol ",i0," `",a,"` (",i0,")")') &
                           i_aerosol, trim(specname(ispec)), ispec; call goErr       
...
!not m7:
  ...
  subroutine calc_properties_mie( c, rh, swbands, lookup, lut, aer, RefracIndex, &
    ...
    use Indices             , only : aerdens, aerhygro
    use Indices             , only : n_aerosol, ispecs_aerosol
    ...
    real, intent(out) ,optional                   :: ext_LUT(:)
    ! --- const ----------------------------------
    ...
    do jband = 1, swbands%n
      ...
      !write (*,*) 'Janot RefracInd jband, lambda[m],' ,trim(specname(ispec)),' : ',jband, 1.0D-6 *swbands%lambda(jband), RefracInd(i_aerosol,jband)
      !write (*,*) 'Janot RefracInd water, lambda[m]:', RefracIndex%Water(jband), 1.0D-6 *swbands%lambda(jband)
    ...
    ! testing ...
    !Santiago commented next 3 lines
    if ( present(sizeparam) ) sizeparam = 0.0
    if ( present(refr_spec) ) refr_spec = (0.0 , 0.0)
    if ( present(extf_spec) ) extf_spec = 0.0
    if ( present(ext_LUT) ) ext_LUT = 0.0
    ...
    do jband = 1, swbands%n
      ...
      do i_aerosol = 1, n_aerosol
        ...
        ! fill:
        !Santiago
        if ( present(ext_LUT) ) ext_LUT(jband) = lu_ext    
        k_ext(i_aerosol) = lu_ext * lambda * lambda * NumberConcentration(i_aerosol)
        k_sca(i_aerosol) = lu_a * k_ext(i_aerosol)
        AF(i_aerosol)    = lu_g
        !write (*,*) 'Janot spec : NumberConcentration,' ,trim(specname(ispec)),' : ', NumberConcentration(i_aerosol)
        ! update sums:
  ...
  subroutine calc_properties_mie_m7( c, rad_wet_m7, rad_dry_m7, water_m7, rh, swbands, lookup, lut, RefracIndex, &
    ...
    do jband = 1, swbands%n
      ...
      do i_aerosol = 1, n_aerosol! this is a bad loop, need to be updated!!!! 
        ...
        i_m7mode = m7_mode(ispec)
        !write(*,*) 'Santiago i_m7mode ',m7_mode
