...
!  real                ::  kf_noise_tau_days
  ...
  ! switch timing:
  call GO_Timer_Switch( itim_init_filter, itim_init_states, status )
  IF_NOTOK_RETURN(status=1)
  ...
  ! switch timing:
  call GO_Timer_Switch( itim_init_states, itim_init_output, status )
  IF_NOTOK_RETURN(status=1)
  ...
  ! switch timing:
  call GO_Timer_Switch( itim_init_output, itim_init_meas, status )
  IF_NOTOK_RETURN(status=1)
    ...
    if ( isfirst ) then
      ...
      if ( kf_with_xm ) then
        ...
        do j = 1, nmodes
          ...
          !imode = imodes(j)
          ! info ...
          write (gol,'("LEKF:   initialize mode : ",i3)') j; call goPr
          ! new or restore ?
          if ( runF%restart ) then
            ! name of mode:
