...
#define IF_NOTOK_RETURN(action) if (status/=0) then; TRACEBACK; action; call goExit(status); end if
#define IF_ERROR_RETURN(action) if (status> 0) then; TRACEBACK; action; call goExit(status); end if
!
#include "lekf.inc"
!
!###############################################################################
...
program LOTOS_EUROS_KF
  ...
  use GO, only : GO_Print_Init, GO_Print_Done, GO_Print_Logfile
  ...
  use LEKF_Meas_OMI_TRC, only : InitMeas_OMI_TRC, DoneMeas_OMI_TRC
  use LEKF_Meas_OMI_TRC, only : GetMeas_OMI_TRC
#endif
  ...
  character(len=*), parameter   ::  rname = 'LOTOS_EUROS_KF'
...
!  real                ::  kf_noise_tau_days
  ...
  write (gol,'("LEKF: init LOTOS-EUROS ...")'); call goPr

  ! init model, return:
  !  o start time, time step
  !  o hours since start
  !  o name of rcfile
  call LE_Model_Init( t, dt, nhour, rcfile, status )
  IF_NOTOK_RETURN(status=1)
  ...
  ! info ...
  write (gol,'("LEKF: setup filter ...")'); call goPr

  ! info ...
  write (gol,'("LEKF:   read flags to enable filter elements ...")'); call goPr

  ! with background run?
  call ReadRc( rcF, 'kf.with.xb', kf_with_xb, status )
  IF_NOTOK_RETURN(status=1)
  ! root only ...
  kf_with_xb = kf_with_xb .and. goc%root

  ! with modes?
  call ReadRc( rcF, 'kf.with.xm', kf_with_xm, status )
  IF_NOTOK_RETURN(status=1)

  ! for testing, model propagation might be skipped:
  call ReadRc( rcF, 'kf.with_propagation', kf_with_propagation, status )
  IF_NOTOK_RETURN(status=1)
  !call ReadRc( rcF, 'kf.with_analysis', kf_with_analysis, status )
  !IF_NOTOK_RETURN(status=1)
  ...
  call ReadRc( rcF, 'kf.noise.dt_hours', kf_noise_dt_hours, status )
  IF_NOTOK_RETURN(status=1)

  call ReadRc( rcF, 'kf.restart', runF%restart, status )
  IF_NOTOK_RETURN(status=1)

  ! to allow reading of dc into xb ...
  call ReadRc( rcF, 'kf.restart.as_forecast', restart_as_forecast, status )
  IF_NOTOK_RETURN(status=1)

  if ( runF%restart ) then
    ! where to read from:
    call ReadRc( rcF, 'kf.restart.path', runF%restart_path, status )
    IF_NOTOK_RETURN(status=1)
    ! file name description:
    call ReadRc( rcF, 'kf.restart.key', runF%restart_key, status )
    IF_NOTOK_RETURN(status=1)
    ...
    if ( restart_as_forecast ) then
      ...
      call ReadRc( rcF, 'kf.restart.xb.from_mean', restart_xb_from_mean, status )
      IF_NOTOK_RETURN(status=1)
      ! ... adhoc: add a standard 'name' to specify the file from which
      !     old data should be read (expcls, volume) :
      ...
      call ReadRc( rcF, 'kf.restart.xb.dc_option'   , restart_xb_dc_option   , status )
      IF_NOTOK_RETURN(status=1)
      call ReadRc( rcF, 'kf.restart.xb.dc.starthour', restart_xb_dc_starthour, status )
      IF_NOTOK_RETURN(status=1)
      call ReadRc( rcF, 'kf.restart.xb.dc.endhour'  , restart_xb_dc_endhour  , status )
      IF_NOTOK_RETURN(status=1)
      call ReadRc( rcF, 'kf.restart.xb.dc.days'     , restart_xb_dc_days     , status )
      IF_NOTOK_RETURN(status=1)
      call ReadRc( rcF, 'kf.restart.xb.dc.dir'      , restart_xb_dc_dir      , status )
      IF_NOTOK_RETURN(status=1)
      call ReadRc( rcF, 'kf.restart.xb.dc.model'    , restart_xb_dc_model    , status )
      IF_NOTOK_RETURN(status=1)
      call ReadRc( rcF, 'kf.restart.xb.dc.expid'    , restart_xb_dc_expid    , status )
      IF_NOTOK_RETURN(status=1)
    else
      ! dummy ...
  ...
  ! write restart files ?
  call ReadRc( rcF, 'kf.restart.save', runF%restart_save, status )
  IF_NOTOK_RETURN(status=1)

  ! write restart files ?
  if ( runF%restart_save ) then
     ! when to write:
     call ReadRc( rcF, 'kf.restart.save.dhour', runF%restart_save_dhour, status, default=24.0 )
     IF_NOTOK_RETURN(status=1)
     ! where to write:
     call ReadRc( rcF, 'kf.restart.save.path', runF%restart_save_path, status )
     IF_NOTOK_RETURN(status=1)
     ! file name description:
     call ReadRc( rcF, 'kf.restart.save.key', runF%restart_save_key, status )
     IF_NOTOK_RETURN(status=1)
  else
     ! dummy ...
  ...
  !call ReadRc( rcF, 'kf.biascorr.surface_ozone', &
  ...
  ! info ...
  write (gol,'("LEKF:   setup states ...")'); call goPr
  ! setup state module:
  ...
  ! init output stuff
  call LEKF_Output_Init( rcfile, t, status )
  IF_NOTOK_RETURN(status=1)

  ! info ...
  write (gol,'("LEKF:   setup measurements ...")'); call goPr
  ...
  ! info ...
  write (gol,'("LEKF: start time loop ...")'); call goPr
  ...
  do
    ...
    ! info ...
    write (gol,'("LEKF: ")'); call goPr
    call wrtgol( 'LEKF: >>> ', t ); call goPr
    write (gol,'("LEKF: ")'); call goPr
    ...
    ! info ...
    call wrtgol('LEKF: prepare timestep ',t,' to ',t+dt); call goPr
    ...
    ! init states ?
    if ( isfirst ) then
      write (gol,'("LEKF: setup for start ...")'); call goPr
      ...
      if ( kf_with_xm ) then
        ...
        do j = 1, nmodes_loc
          ...
          imode = imodes(j)
          ! info ...
          write (gol,'("LEKF:   initialize mode : ",i3," (local ",i0,")")') imode, j; call goPr
          ! new or restore ?
          if ( runF%restart ) then
            ! name of mode:
            write (filekey,'(a,";name=ensm",i3.3)') trim(runF%restart_key), imode
...
#ifdef with_kf_meas_omi_trc
      write (gol,'("LEKF:   load OMI NO2 ...")'); call goPr
      call GetMeas_OMI_TRC( t, t+dt, status )
      IF_NOTOK_RETURN(status=1)
#endif
#ifdef with_kf_meas_modis
...
#ifdef with_kf_meas_omi_trc
    write (gol,'("LEKF:   load OMI NO2 ...")'); call goPr
    call GetMeas_OMI_TRC( t-dt, t, status )
    IF_NOTOK_RETURN(status=1)
#endif
#ifdef with_kf_meas_modis
    write (gol,'("LEKF:   load MODIS ...")'); call goPr
...
!#endif
!#ifdef with_kf_meas_omi_trc
!       ! analyse OMI NO2 measurents; x%m should be recomputed
!       call MeasUpdate_OMI_TRC( t-dt, t, nmodes, status )
!       IF_NOTOK_RETURN(status=1)
!#endif
!#ifdef with_kf_meas_modis
...
!#endif
      ...
      if ( Precisely(t,runF%restart_save_dhour,'hour') ) then
        ...
        if ( kf_with_xm ) then
          ...
          do j = 1, nmodes_loc
            ...
            write (filekey,'(a,";name=ensm",i3.3)') trim(runF%restart_save_key), imode
            ! read state from file:
  ...
  ! info ...
  write (gol,'("LEKF:   done with filter optput ...")'); call goPr
...
! moved to lekf.F90      
  ...
  call LE_Timers_Done( rcfile, status, write_profile=goc%root )
  IF_NOTOK_RETURN(status=1)
