...
!     integer                     ::  varid_date
  ...
  type OMI_TRC_type
    ...
    integer, pointer     ::  time(:)                  ! time of OMI obs
...
contains
  ...
  subroutine InitMeas_OMI_TRC( rcfile, status )
    ...
    ! tracer:
    call ReadRc( rcF, 'kf.meas.omi_trc.tracer', omi_trc%tracer_name, status )
    IF_NOTOK_RETURN(status=1)
    ! search index:
    ...
    ! filename template:
    call ReadRc( rcF, 'kf.meas.omi_trc.filenames', arch_filenames, status )
    IF_NOTOK_RETURN(status=1)
    
    call ReadRc( rcF, 'kf.meas.omi_trc.analyse', omi_trc%analyse, status )
    IF_NOTOK_RETURN(status=1)

    !! testing ....
    !call ReadRc( rcF, 'kf.meas.omi_trc.analyse.order', omi_trc%analyse_order, status )
    !IF_NOTOK_RETURN(status=1)

    ! localization length scale:
    call ReadRc( rcF, 'kf.meas.omi_trc.rho', omi_trc%rho, status )
    IF_NOTOK_RETURN(status=1)

    ! how to define representation error?
    call ReadRc( rcF, 'kf.meas.omi_trc.r.type', omi_trc%r_type, status )
    IF_NOTOK_RETURN(status=1)
    ! switch:
    select case ( trim(omi_trc%r_type) )
      ...
      case ( 'data' )
        ...
        call ReadRc( rcF, 'kf.meas.omi_trc.r.data.scaling', omi_trc%r_data_scaling, status, default=1.0 )
        IF_NOTOK_RETURN(status=1)
      !~ fraction of data:
      case ( 'frac' )
        ! scaling and bounds:
        call ReadRc( rcF, 'kf.meas.omi_trc.r.frac.factor', omi_trc%r_frac_factor, status )
        IF_NOTOK_RETURN(status=1)
        call ReadRc( rcF, 'kf.meas.omi_trc.r.frac.min'   , omi_trc%r_frac_min   , status )
        IF_NOTOK_RETURN(status=1)
        call ReadRc( rcF, 'kf.meas.omi_trc.r.frac.max'   , omi_trc%r_frac_max   , status )
        IF_NOTOK_RETURN(status=1)
      !~
      case default
    ...
    ! screening factor:
    call ReadRc( rcF, 'kf.meas.omi_trc.alfa', omi_trc%screening_factor, status )
    IF_NOTOK_RETURN(status=1)

    ! top of tropo sphere to be used with kernel (Pa):
    call ReadRc( rcF, 'kf.meas.omi_trc.ptropo', omi_trc_ptropo, status )
    IF_NOTOK_RETURN(status=1)

    ! boundary condition variables used above model top:
    call ReadRC( rcF, 'kf.meas.omi_trc.top_hp', omi_trc%top_ph_entry, status )
    IF_NOTOK_RETURN(status=1)
    call ReadRC( rcF, 'kf.meas.omi_trc.top_trc', omi_trc%top_trc_entry, status )
    IF_NOTOK_RETURN(status=1)

    ! pixel range:
    call ReadRc( rcF, 'kf.meas.omi_trc.pixels', line, status )
    IF_NOTOK_RETURN(status=1)
    ! defined?
  ...
  subroutine GetMeas_OMI_TRC( tint1, tint2, status )
    ...
    if ( (tint2 < omi_trc_tr(1)) .or. (omi_trc_tr(2) < tint2) ) then
       ...
       ! info ...
       write (gol,'("KF:     OMI TRC - open file for ",i4,2i2.2)') year, month, day; call goPr
       ...
       ! info ...
       write (gol,'("KF:     OMI TRC - read from file ",a)') fname; call goPr
       ...
       ! info ...
       write (gol,'("KF:     OMI TRC - nr of OMI obs       : ",i6)') nrmeas; call goPr
       ...
       if ( nrmeas > 0 ) then
         ...
         ! info ...
         write (gol,'("KF:     OMI TRC - nr of kernel levels : ",i6)') nrlev; call goPr
       ...
       ! info ...
       write (gol,'("KF:     OMI TRC - file closed ")') ; call goPr

       ! info ...
       if ( (trim(omi_trc%r_type) == 'data') .and. (abs(omi_trc%r_data_scaling-1.0) > 1.0e-5) ) then
         write (gol,'("KF:     OMI TRC - obs error scaling   : ",f6.3)') omi_trc%r_data_scaling; call goPr
       end if
       ...
       do imeas = 1, nrmeas
         ...
         call InDomain( omi_trc%longitude(imeas), omi_trc%latitude(imeas), &
                         runF%dlon, runF%dlat, runF%southb, runF%westb, &
                         omi_trc%ix(imeas), omi_trc%iy(imeas), in_domain )
         ...
         ! increase counter:
         if ( in_domain )  nrmeas_in_domain = nrmeas_in_domain + 1
       ...
       write (gol,*) "KF:   OMI TRC - Nr of obs in domain = ", nrmeas_in_domain;     call goPr
       if ( omi_trc%nmeas > 0 ) then
       ...
       write (gol,*) "KF:   OMI TRC - data already available for this day";     call goPr

    end if

    ! data from file has been read for the current day and is stored at this point
    if ( omi_trc%nmeas == 0 ) then
      write (gol,*) "KF:     OMI TRC - no observations for today ... ";     call goPr
  ...
  !
  ! On return the pointer 'le2omi_vcd_trop' is allocated to 'npix'
  ...
  subroutine CalcMeas_OMI_TRC( le_vmr_grid, le_vmr_aloft, &
    ...
    if ( omi_trc%nmeas < 1 .or. omi_trc%nmeas_intimestep < 1 ) then
       write (gol,'("KF:     OMI TRC - CalcMeas: skip, no observations")'); call goPr
       status = 0; return
    end if

    write (gol,'("KF:     OMI TRC - CalcMeas: compute LE predicted OMI TRC, ",i6," obs ...")') omi_trc%nmeas_intimestep; call goPr
    ...
    if ( omi_trc%nmeas_intimestep /= nmeas_processed ) then
       write (gol,'("KF:  OMI TRC - WARNING CalcMeas: In total ",i6," obs processed")') nmeas_processed ; call goPr
    end if
  ...
  subroutine meas_output_omi_Init( self, rcfile, rckey, status )
    ...
    ! init common stuff:
    call Init( self%com, rcfile, rckey, status )
    IF_NOTOK_RETURN(status=1)
    ...
    IF_NOTOK_RETURN(status=1)
    ! tracer:
    call ReadRc( rcF, trim(rckey)//'.replace', self%replace, status )
    IF_NOTOK_RETURN(status=1)
    ! close:
    call Done( rcF, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine meas_output_omi_PutOut( self, key, t, status, last )
    ...
    if ( omi_trc%nmeas < 1 ) then
       ! info ...
       write (gol,'("KF: OMI TRC - put out return: no TRC obs for today")'); call goPr
       return
    end if
    ! no measurements within timestep ? then leave
    if ( omi_trc%nmeas_intimestep < 1 ) then
       ! info ...
       call wrtgol('KF: OMI TRC - put out return: no TRC obs for time : ',t); call goPr
       return
    end if
    ...
    do ifs = 1, nfs
       ...
       case ( 'xb' )
          ...
          ! simulate omi stuff from le fields:
...
!    writetofile = .true.    
!    do i = 1, omi_trc%nmeas
!       if ( omi_trc%status(i) == 0 ) writetofile = writetofile .and. omi_trc%analysed(i)
!    end do

    if ( .not. writetofile ) then
      write (gol,'("KF: OMI TRC - do not write to file for some reason ...")'); call goPr
  return
    end if
    ...
    ! info ...
    call wrtgol('KF: OMI TRC - Now put out all data for this day at : ',t); call goPr
    ...
    ! define dimensions:
    status = NF90_Def_Dim( self%ncid, 'omi_obs', omi_trc%nmeas, self%dimid_omi_obs )
    IF_NF90_NOTOK_RETURN(status=1)
...
!     self%varid_date = varid
    ...
    status = NF90_Put_Var( self%ncid, self%varid_lon, omi_trc%longitude(1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( self%ncid, self%varid_lat, omi_trc%latitude(1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    ! date in year,month,etc
    status = NF90_Put_Var( self%ncid, self%varid_omi_date, omi_trc%date(:,1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    ...
    status = NF90_Put_Var( self%ncid, self%varid_omi_time, omi_trc%time(1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( self%ncid, self%varid_omi_pressure_levels, omi_trc%pressure_levels(:,1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( self%ncid, self%varid_omi_kernel         , omi_trc%kernel(:,1:omi_trc%nmeas)          )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( self%ncid, self%varid_omi_cloud_radiance_fraction , omi_trc%cloud_radiance_fraction(1:omi_trc%nmeas)  )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( self%ncid, self%varid_y, omi_trc%vcd_trop(1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( self%ncid, self%varid_r, omi_trc%sigma_vcd_trop(1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)

    status = NF90_Put_Var( self%ncid, self%varid_omi_status, omi_trc%status(1:omi_trc%nmeas) )
    IF_NF90_NOTOK_RETURN(status=1)
    ...
    ! write time record:
!     status = NF90_Put_Var( self%ncid, self%varid_time, (/time/), start=(/self%itrec/) )
!     IF_NF90_NOTOK_RETURN(status=1)
    ...
    ! write date record:
!     status = NF90_Put_Var( self%ncid, self%varid_date, reshape(time6,(/6,1/)), &
!          start=(/1,self%itrec/), count=(/6,1/) )
!     IF_NF90_NOTOK_RETURN(status=1)
    ...
    status = NF90_Put_Var( self%ncid, self%varid_le_altitude_levels, omi_trc%le_altitude_levels(:,1:nmeas), &
         start=(/1,1,self%itrec/), count=(/nz+nhz+1,nmeas,1/) )
    IF_NF90_NOTOK_RETURN(status=1)
