...
contains
  ...
  subroutine kfo_meas_Init( self, rcF, rckey, status )
...
#ifdef with_kf_meas_pm
    call Init( self%mo_pm   , rcF, rckey, status )
    IF_NOTOK_RETURN(status=1)
#endif
#ifdef with_kf_meas_pm_m24
    call Init( self%mo_pm_24, rcF, rckey, status )
    IF_NOTOK_RETURN(status=1)
#endif
#ifdef with_kf_meas_aod
    call Init( self%mo_aod, rcF, rckey, status )
    IF_NOTOK_RETURN(status=1)
#endif
#ifdef with_kf_meas_ground
    call Init( self%mo_ground, rcF, rckey, status )
    IF_NOTOK_RETURN(status=1)
#endif
#ifdef with_kf_meas_omi_trc
    call self%mo_omi_trc%Init( rcF, rckey, status )
    IF_NOTOK_RETURN(status=1)
#endif
