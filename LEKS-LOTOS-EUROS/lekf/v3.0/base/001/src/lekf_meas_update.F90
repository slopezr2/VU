  ...
  subroutine LEKF_Meas_Update_Init( self, rcF, status )
    ...
    do idom_y = 1, ndom_y
      do idom_x = 1, ndom_x
        ...
        j0 = (idom_y-1)*ny_loc
        ! init domain:
        call self%dom(idom)%Init( ugg%longitude_1d(1)+i0*ugg%dlon, ugg%dlon, nx_loc, i0, &
                                  ugg%latitude_1d (1)+j0*ugg%dlat, ugg%dlat, ny_loc, j0, status )
        IF_NOTOK_RETURN(status=1)
        ! info ..
...
!        IF_NOTOK_RETURN(status=1)
!        ! testing ..
!        write (gol,'("break after ncdump of HPHR")'); call goErr
!        TRACEBACK; status=1; return
