    ...
    do k = 1, meas_sat%nset
      ...
      if ( msat%analyse ) then
        ...
        if ( nmeas_tot > 0 ) then
          ...
          if ( self%with_v ) then
            ! info ...
            write (gol,'(a," -       generate random numbers ...")') rname; call goPr
          ...
          ! info ...
          write (gol,'(a," -       collect ...")') rname; call goPr
          ! loop over measurements assigned to this domain:
          do ipix = 1, satd%npix
            ! info ...
            if ( modulo(ipix-1,1000) == 0 ) then
              write (gol,'(a," -     pixel ",i0)') rname, ipix; call goPr
            end if
...
!    IF_NOTOK_RETURN(status=1)
  ...
  subroutine LEKF_Meas_Update_LETKF_Update_Ensemble( self, i, j, wa, WWa, status, &
    ...
    if ( any( sum(WWa,dim=2) - 1.0 > 0.001 ) ) then
      ...
      do imode = 1, nmodes
        write (gol,*) WWa(imode,:),' ; ', sum(WWa(imode,:)); call goErr
      end do
      TRACEBACK; status=1; return
    end if
