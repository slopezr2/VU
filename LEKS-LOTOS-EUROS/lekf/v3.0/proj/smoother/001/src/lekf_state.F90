...
module LEKF_State
  ...
  use LE_Output, only : T_LE_Output_CSO_State
#endif
  ...
  type TState
    ...
    type(T_LE_Output_CSO_State), allocatable    ::  sat_state(:) ! (nsat)
#endif
#ifdef with_kf_meas_modis
    ! o modis aod
    integer                   ::  nmodis
...
contains
  ...
  subroutine LEKF_State_Init( rcF, status )
    ...
    if ( kf_with_xb ) then
	  ...
	  write (gol,'(a,":   init xb ...")') rname; call goPr
    ...
    ! with mean/modes ?
    if ( kf_with_xm ) then
	   write (gol,'(a,":   init xm states ...")') rname; call goPr
...
!      end if
      ...
      ! storage for modes:
	  write (gol,'(a,":   init ens ...")') rname; call goPr
  ...
  subroutine LEKF_State_Done( status )
    ...
    ! with mean/modes ?
	write(gol,*) 'triggering lekf_state_done'; call goPr
    if ( kf_with_xm ) then
  ...
  subroutine state_Init( self, name, seed0, status )
    ...
    if ( leo%ncso > 0 ) then
      ! store number:
      self%nsat = leo%ncso
      write (gol,'("self%nsat=",i0," and ncso=",i0)') self%nsat, leo%ncso; call goPr
      ! storage for output stuff:
      allocate( self%sat_state(self%nsat), stat=status )
      IF_NOTOK_RETURN(status=1)
	  write (gol,*) 'Done allocating stuff'; call goPr
      ! loop over files to be written:
      ! ! do k = 1, self%nsat
	  	! ! ! pointer
		! ! write (gol,*) 'Start pointing'; call goPr
		! ! write (gol,*) 'meas_sat', meas_sat%iout(k); call goPr
		! ! satd => leo%cso_data(meas_sat%iout(k))
        ! ! ! setup state (see "le_output.F90" for original):
		! ! write (gol,*) 'requesting initialization'; call goPr
        ! ! call self%sat_state(k)%Init( satd, status, & !key=trim(self%name), &
                                       ! ! description='simulated '//trim(leo%file_cso(k))//' retrievals' )
...
#ifdef with_kf_meas_sat
	write(gol, *) 'reached state done'; call  goPr
    ! any sat output ?
    if ( self%nsat > 0 ) then
      ! loop over files to be written:
      ! ! do k = 1, self%nsat
	    ! ! satd => leo%cso_data(meas_sat%iout(k))
        ! ! ! done with state:
        ! ! call self%sat_state(k)%Done( satd, status )
        ! ! IF_NOTOK_RETURN(status=1)
      ! ! end do  ! output files
      ! clear:
    ...
    if ( inds(0) < 1000 ) then
      ...
      select case ( inds(0) )
        ...
        case ( substate_sat )
          ...
          ! check ...
          if ( (ipix < 1) .or. (ipix > self%sat_state(isat)%sstate%npix) ) then
            write (gol,'("requested ipix=",i0," while npix=",i0," for isat=",i0)') &
                        ipix, self%sat_state(isat)%sstate%npix, isat; call goErr
            TRACEBACK; status=1; return
          end if
		  ! get pointer
		  satd => leo%cso_data(meas_sat%iout(isat))
          ! get value, for now a column is needed, not a profile, add a roadblock somewhere:
          call self%sat_state(isat)%sstate%GetPixel(satd%sdata, ipix, status, y=yloc )
          IF_NOTOK_RETURN(status=1)
		  ! either set to first point or sum...
		  nr=size(yloc)
		  !value_aux = sum(yloc(1:satd%sdata%nr))
		  value_aux = sum(yloc(1:nr))
		  value=value_aux
#endif
