...
!  end subroutine AnDom_BCast_Modes
  ...
  subroutine AnDom_Fill_HPHR_etc( self, status )
    ...
    if ( status /= 0 ) then
	  ...
	  do
        !! info ...
        write (gol,'("TESTING - increase diagonal ...")'); call goPr
        !! reset:
        self%HPHR = A
        ...
        !~ set added value of loop_nr * 0.5% of maximum for diagonal elements:
        do iobs = 1, self%nobs
         ! self%HPHR(iobs,iobs) = max( 0.005*s2max*loop_nr, self%HPHR(iobs,iobs) )
         self%HPHR(iobs,iobs) = 0.005*s2max*loop_nr + self%HPHR(iobs,iobs)
        end do
        ! dump:
        ...
        call LinAlg_Sym_Factorize( self%HPHR, self%U, status )
        ! info ...
        write (gol,'("TESTING - loop: ",f8.2)') loop_nr; call goPr
        write (gol,'("TESTING - return status factorization: ",i0)') status; call goPr
		loop_nr = loop_nr + 1
		if ((status==0) .or. (loop_nr>10)) exit
	  end do
	  if (loop_nr>10) then
		! leave if more than ten loops:
		TRACEBACK; status=1; return
	  end if
    end if
...
!      ! testing ...
  ...
  subroutine AnDom_Fill_Y_etc( self, lon, lat, xcorr, status, &
    ...
    if ( self%nobsl > 0 ) then
      ...
      !! testing ...
      !if ( dbg ) then
      !write (gol,*) 'Santiago en calculo wa ' ; call goPr
