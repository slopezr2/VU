...
!    integer       ::  i0, j0
    ...
    write (gol,'(a," - init analysis method ...")') rname; call goPr
     ...
     ! Read if concentrations will be assimilated:
    call rcF%Get( 'kf.meas.conc_assimi', conc_assimilation, status )
    IF_NOTOK_RETURN(status=1)
    if (conc_assimilation) then
        write (gol,'(a," -   Concentrations will be assimilated: ")') rname; call goPr
    else
        write (gol,'(a," -   Concentrations will not be assimilated: ")') rname; call goPr
    endif
    ! form key:
...
!    integer                               ::  displ
    ...
    real                                  ::  bbox(4)
    integer                               ::  glbid
	real,pointer                          ::  yloc(:), rloc(:,:),obs_error(:)
	integer                        ::  nr
#endif
      ...
      if ( msat%analyse ) then
        ...
        if ( nmeas_tot_loc > 0 ) then
          ...
          do ipix = 1, satd%sdata%npix
            ...
            ! extract measurement data: Santiago changed yr by profile, an erased R=rloc
            call satd%sdata%GetPixel( ipix, status, glbid=glbid, lon=lon, lat=lat, profile=yloc, name='yr',name_error='obs_error',obs_error=obs_error)
            IF_NOTOK_RETURN(status=1)
			...
			y = sum(yloc(1:nr))
			! same for the covariance/std, keep option for future profiles Santiago add r = 0.001
			r = sum(obs_error(1:nr))
			r=r*1e0
			if (r <= 0.0) then
			    r=1e-2
			end if
			!r = 1e-3
			!Santiago commented next lines
 			!do i = 1,nr
 			!	r = r + obs_error(i)
 			!end do
...
!    IF_NOTOK_RETURN(status=1)
  ...
  subroutine LEKF_Meas_Update_LETKF_Apply( self, t1, t2, status )
    ...
    if ( self%andom%nobs > 0 ) then
      ...
      ! info ...
      !Santiago acá leer si se quiere asimilar concentraciones, solo ejecutar si sí se quiere
        if (conc_assimilation) then
            write (gol,'(a," -     analyse concentrations ...")')  rname; call goPr
        else
            write (gol,'(a," -    skip concentrations ...")')  rname; call goPr
        end if ! conc_assimilation
  ...
  subroutine LEKF_Meas_Update_LETKF_Update_Cell( self, xi, wa, WWa, xi_a, xx, status )
    ...
    !! testing ..
    !write (gol,*) 'Santiago en LETKF_update_Cell '; call goPr
    !write (gol,*) 'wa ; <wa> = '; call goPr
    !write (gol,*) wa, ' ; ', sum(wa)/size(wa); call goPr 
