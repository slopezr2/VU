...
!  use LEKF_state, only : imodes
  ...
  use LEKF_Noise, only : SetDC, ResetDC, SetDC_Means,adjust_random_values
...
!  real                ::  kf_noise_tau_days
  ...
  character(len=256)  ::  read_ens_dc_dir
  character(len=32)   ::  read_ens_dc_model
  character(len=32)   ::  read_ens_dc_expid
  type(TDate)         ::  t_dc
  integer             ::  iday
  real                ::  dc_weight
  real, allocatable   ::  w(:,:)
  !real, allocatable   ::  noise_mu(:)
  !real   ::  w_mean
  !real   ::  w_in
  ...
  !Santiago to allow each ensemble reads dc from file
  call rcF%Get( 'kf.read_dc_file', read_dc_file, status )
  IF_NOTOK_RETURN(status=1) 
  if (read_dc_file) then
      call rcF%Get( 'kf.read_dc_file.path', read_ens_dc_dir, status )
      IF_NOTOK_RETURN(status=1)
      call rcF%Get( 'kf.read_dc_file.model', read_ens_dc_model, status )
      IF_NOTOK_RETURN(status=1)
  endif
  ...
  IF_NOTOK_RETURN(status=1)
  write (gol,'(a,":   finished setup states ...")') rname; call goPr
  ! switch timing:
  ...
  do
    ...
    if ( isfirst ) then
      ...
      ! ensemble present?
      if ( kf_with_xm ) then
        ! pull random values and normalize
        allocate(w(nnoise,nmodes))
        !allocate(noise_mu(nnoise))
        ...
        do j = 1, nmodes
          ...
          if ( runF%restart ) then
            ...
            !Santiago, dejar esto para que las concentraciones inicien donde terminaron las pasadas
            ...
            IF_NOTOK_RETURN(status=1)
            ! set mean values:
            call SetDC_Means( Ens(j)%dc(:,:,:,1),j, status )
            IF_NOTOK_RETURN(status=1)
            ! set new noise
             ! allocate( w(nmodes) )
           end if
        ...
        if ( read_dc_file ) then
            ! Get path and names of the ensemble dc files:
            write (gol,*) 'Ensembles dc factor will be readed from files'; call goPr
            
        else
            do j = 1, nmodes
               do i = 1,nnoise
                   call Ens(j)%rnd%Get_Normal( w(i,j), status )
                   IF_NOTOK_RETURN(status=1)
               end do
            end do
            call adjust_random_values(w,nnoise,nmodes,status,start=1)
            IF_NOTOK_RETURN(status=1)
        endif
        ...
        do j = 1, nmodes
            if ( read_dc_file ) then
               write (read_ens_dc_expid,'("xi_",i2.2)') j
               write(*,*) read_ens_dc_expid
               call ReadDC( read_ens_dc_dir, read_ens_dc_model, read_ens_dc_expid,&
                               t%year,t%month,t%day, dc_lag, dc_nt, dc_day, status )
               IF_NOTOK_RETURN(status=1)
               Ens(j)%dc(:,:,:,1) = dc_day(:,:,:,t%hour+1)
            else
                do i = 1, nnoise
                  ! calculate mean
                  !w_mean = sum(w(i,1:nmodes))/nmodes + noise_mu(i)
                  !w_in = (w(i,j)+ noise_mu(i)) / w_mean ! normalize before passing onward
                  ! remove noise_mu
                  !w_in = w_in - noise_mu(i)
                  !write (gol,*) 'w test2:',i,w(i,1:nmodes),'nmodes',nmodes; call goPr
                  !write (gol,*) 'w in2:',i,j,w_in,'nmodes',nmodes; call goPr
                  ! random factor around mean; truncate negative values:
                  call SetDC(w(i,j), Ens(j)%dc, i, status )
                  IF_NOTOK_RETURN(status=1)
            end do
            end if
        end do
        ...
        IF_NOTOK_RETURN(status=1)
        !
		write (gol,*) 'Done with first timestep/analysis xm'; call goPr
        deallocate(w)
        !deallocate(noise_mu)
      end if ! with xm
      ...
      if ( kf_with_xb ) then
        ...
        if ( runF%restart ) then
          ...
          call SetDC_Means( xb%dc(:,:,:,1),0, status )
          IF_NOTOK_RETURN(status=1)
        end if
		write (gol,*) 'Done with first timestep/analysis xb'; call goPr
      end if  ! with xb
      ...
      IF_NOTOK_RETURN(status=1)
	  write (gol,*) 'Done forecast putout'; call goPr
      call LEKF_Output_PutOut( 'analysis', t,status )
      IF_NOTOK_RETURN(status=1)
	  write (gol,*) 'Done analysis putout'; call goPr
    ...
    IF_NOTOK_RETURN(status=1)
    ! allocate normal distribution array
    ...
    if ( Precisely(t,kf_noise_dt_hours,'hour') ) then
       ...
       if ( kf_with_xm .and. (.not. isfirst) ) then
          !Santiago poner un if aca mas grande que pregunte si es restart, si lo es entonces READ cada ensemble con ReadDC
          ! shift history:
            if (read_dc_file) then
                allocate( dc_day(nx,ny,nnoise,dc_nt) )
                write (gol,*) 'Ensembles dc factor will be readed from files'; call goPr
	   ...
	   write (gol,*) 'Done with next timestep/analysis xm'; call goPr
         ...
         if ( runF%restart .and. restart_as_forecast ) then
           ...
           select case ( trim(restart_xb_dc_option) )
             ...
             case ( 'hour24_shelley' )
               ...
               t_dc = t
               ! info
               write (gol,'("SHELLEY    --> xb%dc from hour ",i2," (",i4,2("-",i2.2),")")') &
                                  t%hour+1, t_dc%year,t_dc%month,t_dc%day; call goPr
               ! storage:
               allocate( dc_day(nx,ny,nnoise,dc_nt) )
               ! read dc from file:
               call ReadDC( restart_xb_dc_dir, restart_xb_dc_model, restart_xb_dc_expid,&
    ...
    ! filter application ?
    if ( kf_with_xm ) then
	  write (gol,*) 'Next update with next timestep/analysis xm'; call goPr
      ! measurement update:
      call Meas_Update%Apply( t-dt, t, status )
      IF_NOTOK_RETURN(status=1)
		write (gol,*) 'Done update with next timestep/analysis xm'; call goPr
!#ifdef with_kf_meas_maori
...
!#endif
    ...
    ! wait for all processes to finish analysis,
    ! some might be very busy if they have a lot of observations ...
    call goc%Barrier( status )
    IF_NOTOK_RETURN(status=1)
	...
	write (gol,*) 'Next output with next timestep/analysis xm'; call goPr
    call LEKF_Output_PutOut( 'analysis', t, status, last=the_end )
    IF_NOTOK_RETURN(status=1)
	write (gol,*) 'Done output with next timestep/analysis xm'; call goPr
