    ...
    if ( mdata%obs_nvar > 0 ) then
      ...
      select case ( trim(obs_type) )
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~              
          ...
          ! same file ?
          if ( trim(fname) == trim(mdata%obs_file) ) then
            write (gol,'("expecting csv obs files to be different per time step, but requested again:")'); call goErr
            write (gol,'("  ",a)') trim(mdata%obs_file); call goErr
            TRACEBACK; status=1; return
          end if
          ...
          if ( .not. exist ) then
            ...
            do
              ! read record:
              call obs_CSV%ReadRecord( station_code, obsvname, val, unit, status )
              if ( status < 0 ) then
                ! end of file:
                exit
          ...
          ! now read every interval again, might be the same data as before
            ...
            do ivar = 1, mdata%obs_nvar
              ...
              if ( nloc_found == 0 ) then
                ...
                !TRACEBACK; status=1; return
                !Give a warning instead of an error when some or all observations are missing
                write (gol,'(" WARNING - no valid locations found, probably something wrong with filename expansion")'); call goPr
                write (gol,'(" WARNING - or just no available data ...")'); call goPr
                write (gol,'(" WARNING - add `allow_missing=F` to the query to list the files that could not be found")'); call goPr
              end if
