...
!          end if ! loc id range defined
          ...
          do iloc = 1, nloc
            ...
            do obs_ivar = 1, obs_nvar
              ...
              !! testing ...
              !write (gol,*) '       xxx iloc=',iloc,'; y=', y; call goPr
...
!    end if
  ...
  subroutine AnDom_Exchange_Nearby( self, status )
    ...
    do id = 0, goc%npes-1
      if ( id == goc%id ) cycle
      write (gol,'(a," -   found ",i4," observations nearby domain ",i3)') rname, nsend(id), id; call goPr
    end do
...
!        TRACEBACK; status=1; return
    ...
    if ( status /= 0 ) then
      ...
      ! target file:
      !write (dumpfile,'("HPHR__pe",i0,".nc")') goc%myid
      !write (dumpfile,'("HPHR__pe",i0,".nc")') 'XXX'
      dumpfile="HPHR__pe.nc"
      ! info ...
      write (gol,'("Factorization failed, write matrix to:")'); call goErr
