...
contains
  ...
  subroutine LEKF_TimeStep_Run( t, dt, x, is_model, status, xb )
    ...
    real, allocatable    ::  caloft_save  (:,:,:)      ! dims (nx,ny,nspec)
    ...
    allocate( caloft_save  (nx,ny,nspec) )
    ...
    do inoise = 1, nnoise
      ...
      select case ( trim(noise_name(inoise)) )
        ...
        case ( 'o3_top' )
          ...
          caloft(:,:,i_o3) = caloft(:,:,i_o3) * x%dc(:,:,inoise,1) * dfc(inoise)
        ...
        case ( 'PM25_BC_emis' )
          ...
          do iz = 1, nz
            ...
            emis_a(:,:,iz,i_ec_ff ) = emis_a(:,:,iz,i_ec_ff ) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_ec_wb ) = emis_a(:,:,iz,i_ec_wb ) * x%dc(:,:,inoise,1) * dfc(inoise)
            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              ...
              bc_west (:,iz,i_ec_ff ) = bc_west (:,iz,i_ec_ff ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_ec_wb ) = bc_west (:,iz,i_ec_wb ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              ...
              bc_east (:,iz,i_ec_ff ) = bc_east (:,iz,i_ec_ff ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_ec_wb ) = bc_east (:,iz,i_ec_wb ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              ...
              bc_south(:,iz,i_ec_ff ) = bc_south(:,iz,i_ec_ff ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_ec_wb ) = bc_south(:,iz,i_ec_ff ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              ...
              bc_north(:,iz,i_ec_ff ) = bc_north(:,iz,i_ec_ff ) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_ec_wb ) = bc_north(:,iz,i_ec_wb ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels
    ...
    if ( .not. is_model ) then
      ...
      do inoise = 1, nnoise
        ...
        select case ( trim(noise_name(inoise)) )
          ...
          ! already done ...
          case ( 'None' )
          case ( 'NOx_emis', 'VOC_emis', 'SOx_emis', 'NH3_emis', 'PM25_BC_emis'  )
          case ( 'o3_top', 'o3_vd' )
          case ( 'dust_bc' )
  ...
  subroutine LEKF_CalcMeas( x, t, status )
    ...
    use LE_Bound         , only : caloft
    use LEKF_Meas_OMI_TRC, only : CalcMeas_OMI_TRC
    use LEKF_Meas_OMI_TRC, only : omi_trc_itracer
#endif
  ...
  subroutine TruncateStates( status )
    ...
    do j = 1, nmodes
      ...
      IF_NOTOK_RETURN(status=1)
#ifdef with_kf_meas_omi_trc
       if ( Ens(j)%n_omi_trc > 0 ) then
         Ens(j)%omi_trc = max( 0.0, Ens(j)%omi_trc )
       end if
#endif
#ifdef with_kf_meas_modis
...
#ifdef with_kf_meas_omi_trc
       ! simulate omi fields:
       call CalcMeas_OMI_TRC( Ens(j)%c(:,:,:,omi_trc_itracer), caloft(:,:,omi_trc_itracer), &
                                Ens(j)%n_omi_trc, Ens(j)%omi_trc, status )
       IF_NOTOK_RETURN(status=1)
#endif
