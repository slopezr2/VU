    ...
    if ( isfirst ) then
      ...
      if ( kf_with_xm ) then
        ...
        do j = 1, nmodes
          ...
          !call Pull_random_values(w,nnoise,nmodes,Ens(j)%rnd,status,start=1)
...
!            end do ! modes
    ...
    if ( Precisely(t,kf_noise_dt_hours,'hour') ) then
       ...
       if ( kf_with_xm .and. (.not. isfirst) ) then
          ...
          !call Pull_random_values(w,nnoise,nmodes,Ens(j)%rnd,status)
          ! call adjust_random_values(w,nnoise,nmodes,status)
          ! IF_NOTOK_RETURN(status=1)
          do i = 1, nnoise
!              do j = 1, nmodes
              !  
              ...
              do j = 1, nmodes
                ...
                ! remove noise_mu
                !w_in = w_in - noise_mu(i)
                !write (gol,*) 'w test:',i,w(i,1:nmodes),'nmodes',nmodes; call goPr
                !write (gol,*) 'w in:',i,j,w_in,nmodes; call goPr
