...
module LEKF_Noise
  ...
  public  ::  noise_mini, noise_maxi !, noise_sigma, noise_mu

  public  ::  LEKF_Noise_Init, LEKF_Noise_Done
  public  ::  SetDC, ResetDC,adjust_random_values
  public  ::  SetDC_Means
  public  ::  ClipDC
...
contains
  ...
  subroutine LEKF_Noise_Init( rcF, status )
    ...
    ! read if spatial correlation will be used
    call rcF%Get( 'kf.noise.spatial_correlation', spatial_corre, status )
  ...
  subroutine SetDC_Means( dc,j, status )
    use Num      , only : T_Random !Santiago
    use dims          , only : nx, ny
    use GO     , only : goc
    ...
    integer, intent(in)              ::  j !Number of Ensemble, 0 to xb
    integer, intent(out)              ::  status
    ...
    integer       ::  inoise,inx,jny,isteps,nsteps=5,rand_int
    real            ::  mean_dc,std_dc,rand_real
    type(T_Random) :: random_num !Santiago
    integer               ::  shp(2)
    real, allocatable     ::  glb_dc(:,:,:),glb_dc_backup(:,:)

    ! --- begin --------------------------------
    !Santiago
    call random_number(rand_real)
    rand_int=int(rand_real*100*j)
    call random_num%Init(rand_int,status)
    ...
    ! storage:
    if ( goc%root ) then
      allocate( glb_dc(shp(1),shp(2),nnoise), stat=status )
      IF_NOTOK_RETURN(status=1)
      allocate( glb_dc_backup(shp(1),shp(2)), stat=status )
      IF_NOTOK_RETURN(status=1)
    else
      allocate( glb_dc(1,1,nnoise), stat=status )
      IF_NOTOK_RETURN(status=1)
      allocate( glb_dc_backup(1,1), stat=status )
      IF_NOTOK_RETURN(status=1)
    end if

    ! collect:
    !call dom%GatherV( dc, glb_dc, status )
    !IF_NOTOK_RETURN(status=1)
    ...
    !Santiago
        if (spatial_corre) then
            if (j==0) then 
                ! loop over noise parameters:
                if ( goc%root ) then
                    do inoise = 1, nnoise
                        ! set to mean values:
                        glb_dc(:,:,inoise) = noise_mu(inoise)
                    end do
                end if
            else
                do inoise = 1, nnoise
                    if ( goc%root ) then
                        do inx=1,shp(1)
                            ! set to mean values:
                            call random_num%Get_Normal(glb_dc(inx,:,inoise),status,mu=1.0,sigma=10.0)
                        end do

                        do isteps=1,nsteps
                            glb_dc_backup=glb_dc(:,:,inoise)
                            do inx=0,shp(1)-1
                                do jny=0,shp(2)-1
                                    call getmeanneigh(glb_dc_backup,inx,jny,2,glb_dc(inx+1,jny+1,inoise))
                                end do !inx
                            end do !jny
                        end do!isteps
                        glb_dc(:,:,inoise)=exp(glb_dc(:,:,inoise))
                        mean_dc=sum(glb_dc(:,:,inoise))/(shp(1)*shp(2))
                        std_dc=sqrt (sum((glb_dc(:,:,inoise)-mean_dc)**2) / (shp(1)*shp(2)))
                        glb_dc(:,:,inoise)=noise_sigma(inoise)*((glb_dc(:,:,inoise)-mean_dc)/std_dc )+noise_mu(inoise)
                        !Here something like
                    end if !calculate just the root
                    call dom%ScatterV( glb_dc(:,:,inoise), dc(:,:,inoise), status )
        ...
        end if !If spatial correlation is used
    ! ok
  ...
  ! ***
    !subroutine Pull_random_values(w,mu_out,rnd,ind, status)
    subroutine adjust_random_values(w,nnoise,nmodes,status,start)
    ...
    character(len=*), parameter   ::  rname = mname//'/PullValues'
    
    ! --- local --------------------------------
    
    integer      ::  i,j
    real      ::  w_tmp(nmodes)
    real      ::  w_ini(nnoise,nmodes)
    real      ::  w_max,w_min,w_offset
    ! real      ::  alf(nx,ny)
    
    ! --- begin --------------------------------
    ! allocate(w_tmp(nmodes))
    ! allocate(w_ini(nnoise,nmodes))
    ! do i = 1, nnoise
      ! do j = 1, nmodes
        ! ! draw number from normal distribution:
        ! call rnd%Get_Normal( w(i,j), status )
        ! IF_NOTOK_RETURN(status=1)
      ! end do
    ! end do
    ! 
    w_ini(:,:) = w(:,:)
    !--> check that these numbers fall within the range min/max
    ! mostly important for the first values, later on can be released, remove potential high bias due to 0,nmax cap.
    if ( present(start) ) then
        do i = 1, nnoise
            !write (gol,*) 'w(i,:) start',w_ini(i,:); call goPr
            ! check top end for truncate, first top as the differences will be smaller than at the lower end in most case 0lower,1mean,3>higher
            w_tmp(:) = noise_mu(i) + (w_ini(i,:) * noise_sigma(i))
            w_max = maxval(w_tmp)
            w_min = minval(w_tmp)
            w_offset = sum(w_tmp)/nmodes - noise_mu(i)
            !write(gol,*) 'start max',w_max,'min',w_min,'offset',w_offset; call goPr
            if (w_max - w_offset .gt. noise_maxi(i)) then
                ! set to offset, shifting distribution and making std only a slight bit smaller
                do j = 1,nmodes
                    if (w_tmp(j) - w_offset >noise_maxi(i)) then
                    ! subtract a doubleof offset, shifting distribution and making std only a slight bit smaller Santiago change new std to 0.9
                        w_ini(i,j) = noise_maxi(i)+(w_offset / noise_sigma(i))*0.999
                    end if
                    ! create new offset
                end do
                w_tmp(:) = noise_mu(i) + (w_ini(i,:) * noise_sigma(i))
                ...
                w_min = minval(w_tmp)
                w_offset = sum(w_tmp)/nmodes - noise_mu(i)
                ! check that now its correct
                if (w_max - w_offset .lt. noise_maxi(i)) then
                    w_ini(i,1:nmodes) = w_ini(i,1:nmodes) - (w_offset/noise_sigma(i))
                else
                    write (gol,*) 'still found too high value in distribution, check',w_ini(i,1:nmodes); call goErr
                    TRACEBACK; status=1; return
                end if 
            end if
            !write (gol,*) 'w(i,:) after top',w(i,:); call goPr
            w_tmp(:) = noise_mu(i) + (w_ini(i,:) * noise_sigma(i))
            w_max = maxval(w_tmp)
            w_min = minval(w_tmp)
            w_offset = sum(w_tmp)/nmodes - noise_mu(i)
            !write(gol,*) 'after top max',w_max,'min',w_min,'offset',w_offset; call goPr
            ! check if all larger than zero
            if (w_min - w_offset .gt. noise_mini(i)) then
                w_ini(i,1:nmodes) = w_ini(i,1:nmodes) - w_offset  / noise_sigma(i)
            else
                do j = 1,nmodes
                    if (w_tmp(j) - w_offset .lt. noise_mini(i)) then
                    ! set to offset, shifting distribution and making std only a slight bit smaller
                        w_ini(i,j) = noise_mini(i) - (w_offset / noise_sigma(i))
                    end if
                    ! create new offset
                end do
                w_tmp(:) = noise_mu(i) + (w_ini(i,:) * noise_sigma(i))
                ...
                if (w_min - w_offset .gt. noise_mini(i)) then
                    ...
                    !Santiago Added next lines
                    write (gol,*) 'still found negative value in distribution, check',w_ini(i,1:nmodes)!; call goErr
                    !TRACEBACK; status=1; return
                    do j = 1,nmodes
                        if (w_tmp(j) - w_offset .lt. noise_mini(i)) then
                            ! Santiago Set w_ini to obtain noise_mini value
                            w_ini(i,j) = (noise_mini(i)-noise_mu(i))/noise_sigma(i)
                        end if
                    ! create new offset
                    end do
                end if 
            ...
            w_min = minval(w_tmp)
            w_offset = sum(w_tmp)/nmodes - noise_mu(i)
            !write(gol,*) 'after bot max',w_max,'min',w_min,'offset',w_offset; call goPr
            !write (gol,*) 'w(i,:) after bot',w(i,:); call goPr
  ...
  end subroutine adjust_random_values

  
  subroutine SetDC(w, dc, ind, status, dt_days )
    ...
    ! ~ init to mean:
    !dc(1:nx,1:ny,ind,1) = noise_mu(ind)
    !write (gol,*) 'dc-5a:', dc(5,5,ind,1),w,noise_mu(ind); call goPr
    ...
    if ( present(dt_days) ) then
      ...
      !write (gol,*) 'dc-5alf:',  dc(5,5,ind,2),dc(5,5,ind,1),w,noise_mu(ind),alf(5,5); call goPr
      dc(:,:,ind,1) = dc(:,:,ind,1) + alf * (dc(:,:,ind,2)-noise_mu(ind))
    else
      ! no time correlation:
      alf = 0.0
      !write (gol,*) 'dc-5:', dc(5,5,ind,1),w,noise_mu(ind); call goPr
    end if

    ! ~ add noise:
    dc(:,:,ind,1) = dc(:,:,ind,1) + sqrt(1.0-alf**2) * noise_sigma(ind) * w
    !dc(:,:,ind,1) = dc(:,:,ind,1) + sqrt(1.0-alf**2) * w
    !write (gol,*) 'dc-5c:', dc(5,5,ind,1),w,noise_mu(ind); call goPr
    ! truncate to [mini,maxi] :
