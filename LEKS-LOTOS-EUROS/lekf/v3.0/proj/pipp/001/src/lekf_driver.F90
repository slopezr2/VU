    ...
    do inoise = 1, nnoise
      ...
      select case ( trim(noise_name(inoise)) )
        ...
        case ( 'dust_emis' )
          ...
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_dust_ccc) = emis_a(:,:,iz,i_dust_ccc) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_cc) = emis_a(:,:,iz,i_dust_cc) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_c) = emis_a(:,:,iz,i_dust_c) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_ff ) = emis_a(:,:,iz,i_dust_ff ) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_f ) = emis_a(:,:,iz,i_dust_f ) * x%dc(:,:,inoise,1) * dfc(inoise)

            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_dust_ccc) = bc_west (:,iz,i_dust_ccc) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_cc) = bc_west (:,iz,i_dust_cc) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_c) = bc_west (:,iz,i_dust_c) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_ff ) = bc_west (:,iz,i_dust_ff ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_f) = bc_west (:,iz,i_dust_f) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_ccc) = bc_east (:,iz,i_dust_ccc) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_cc) = bc_east (:,iz,i_dust_cc) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_c) = bc_east (:,iz,i_dust_c) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_ff ) = bc_east (:,iz,i_dust_ff ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_f ) = bc_east (:,iz,i_dust_f ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_ccc) = bc_south(:,iz,i_dust_ccc) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_cc) = bc_south(:,iz,i_dust_cc) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_c) = bc_south(:,iz,i_dust_c) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_ff ) = bc_south(:,iz,i_dust_ff ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_f ) = bc_south(:,iz,i_dust_f ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_ccc) = bc_north(:,iz,i_dust_ccc) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_cc) = bc_north(:,iz,i_dust_cc) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_c) = bc_north(:,iz,i_dust_c) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_ff ) = bc_north(:,iz,i_dust_ff ) * x%dc( :,ny,inoise,1) * dfc(inoise)
        ...
        case ( 'Na_emis' )
          ...
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_na_ccc) = emis_a(:,:,iz,i_na_ccc) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_cc) = emis_a(:,:,iz,i_na_cc) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_c) = emis_a(:,:,iz,i_na_c) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_ff ) = emis_a(:,:,iz,i_na_ff ) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_f ) = emis_a(:,:,iz,i_na_f ) * x%dc(:,:,inoise,1) * dfc(inoise)

            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_na_ccc) = bc_west (:,iz,i_na_ccc) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_cc) = bc_west (:,iz,i_na_cc) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_c) = bc_west (:,iz,i_na_c) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_ff ) = bc_west (:,iz,i_na_ff ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_f) = bc_west (:,iz,i_na_f) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_ccc) = bc_east (:,iz,i_na_ccc) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_cc) = bc_east (:,iz,i_na_cc) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_c) = bc_east (:,iz,i_na_c) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_ff ) = bc_east (:,iz,i_na_ff ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_f ) = bc_east (:,iz,i_na_f ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_ccc) = bc_south(:,iz,i_na_ccc) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_cc) = bc_south(:,iz,i_na_cc) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_c) = bc_south(:,iz,i_na_c) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_ff ) = bc_south(:,iz,i_na_ff ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_f ) = bc_south(:,iz,i_na_f ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_ccc) = bc_north(:,iz,i_na_ccc) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_cc) = bc_north(:,iz,i_na_cc) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_c) = bc_north(:,iz,i_na_c) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_ff ) = bc_north(:,iz,i_na_ff ) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_f ) = bc_north(:,iz,i_na_f ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels


        case('aerosol_emis')
            aerosoles=(/i_ppm_f,i_ppm_c,i_ec_c,i_ec_f,i_dust_ccc,i_dust_cc,i_dust_c,i_dust_ff,i_dust_f,i_pom_c,i_pom_f,i_na_ccc,i_na_cc,i_na_c,i_na_ff,i_na_ff /)
