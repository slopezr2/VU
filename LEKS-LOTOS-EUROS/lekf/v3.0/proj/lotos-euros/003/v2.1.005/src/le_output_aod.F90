...
contains
  ...
  subroutine LE_Output_AOD_Init( leo, rcF, rckey, typ, name, state, status )
    ...
    ! pack output?
    call rcF%Get( trim(basekey)//'.pack_output', leo%pack_output, status )
    IF_NOTOK_RETURN(status=1)

    ! write bias corrected aod ?
    call rcF%Get( trim(basekey)//'.biascorr', leo%write_biascorr, status )
    IF_NOTOK_RETURN(status=1)
