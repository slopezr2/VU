...
contains
  ...
  subroutine dat_Init( dat, rcF, rckey, typ, status )
    ...
    ! archive location:
    call rcF%Get( trim(rckey)//'.'//trim(typ)//'.archive', dat%arch_dir, status )
    IF_NOTOK_RETURN(status=1)

    ! space seperated list with platform names:
    call rcF%Get( trim(rckey)//'.'//trim(typ)//'.platforms', key, status )
    IF_NOTOK_RETURN(status=1)
    ...
    do i = 1, dat%nplatform
      ! read product name:
      call rcF%Get( trim(rckey)//'.'//trim(typ)//'.'//trim(dat%platforms(i))//'.product', dat%products(i), status )
      IF_NOTOK_RETURN(status=1)
    end do
    
    ! error std.dev. fraction:
    call rcF%Get( trim(rckey)//'.'//trim(typ)//'.sigma_frac', dat%sigma_frac, status )
    IF_NOTOK_RETURN(status=1)
    
    ! analyse this set ?
    call rcF%Get( trim(rckey)//'.'//trim(typ)//'.analyse', dat%analyse, status )
    IF_NOTOK_RETURN(status=1)
