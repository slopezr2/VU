...
!      end if
    ...
    ! read files:
    call rcF%Get( trim(leo%rckey)//'.aod.files', list, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine leos_Init( leos, leo, rcF, name, status )
    ...
    use SA_Labeling            , only : label_emissions
#endif        
#ifdef with_radiation
    ...
    if ( leo%nemislabel > 0 .and. label_emissions ) then
      ! storage for output stuff:
    ...
    if ( leo%naod > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%naod
        call LE_Output_AOD_Init( leos%output_aod(i), rcF, &
