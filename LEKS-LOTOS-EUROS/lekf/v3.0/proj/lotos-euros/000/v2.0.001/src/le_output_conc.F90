...
contains
  ...
  subroutine LE_Output_Conc_PutOut( leo, t, c, cg, status )
    ...
    if ( (t < leo%tr(1)) .or. (leo%tr(2) < t) ) then
      ...
      call LE_Output_Define_Dims_Lon_Lat(leo%ncid, leo%dimid_lon, leo%dimid_lat, &
                                         leo%ni+2*leo%nhx, leo%nj+2*leo%nhy, trim(leo%com%CF_convention), status)
      IF_NOTOK_RETURN(status=1) 
    ...
    if ( leo%itrec == 1 ) then
      ...
      ! write longitudes:
      allocate( lons(leo%i1-leo%nhx:leo%i2+leo%nhx) )
      do i = leo%i1-leo%nhx, leo%i2+leo%nhx
        lons(i) = runF%westb + (i-0.5)*runF%dlon
      end do
      
      allocate( lats(leo%j1-leo%nhy:leo%j2+leo%nhy) )
      ...
      call LE_Output_Put_Var_Lon_Lat( leo%ncid, leo%varid_lon, leo%varid_lat, &
                                      leo%ni+2*leo%nhx, leo%nj+2*leo%nhy, &
                                      lons, lats, trim(leo%com%CF_convention), status)
      IF_NOTOK_RETURN(status=1)
  ...
  subroutine Write_GrADS_Ctl( leo, status )
    ...
    if ( leo%grads_ctl ) then
      ...
      IF_NOTOK_RETURN(status=1)
      ! xdef:
      call GrADS_Ctl_XYDef( ctl, 'XDEF', leo%ni+2*leo%nhx, leo%westb +(0.5-leo%nhx)*runF%dlon, runF%dlon, status )
      IF_NOTOK_RETURN(status=1)
      ! ydef:
      call GrADS_Ctl_XYDef( ctl, 'YDEF', leo%nj+2*leo%nhy, leo%southb+(0.5-leo%nhy)*runF%dlat, runF%dlat, status )
      IF_NOTOK_RETURN(status=1)
      ! zdef:
      select case ( trim(leo%levtype) )
        case ( 'levels' )
