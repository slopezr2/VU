...
contains
  ...
  subroutine Get_SWG_Data( fname, units, swg, status )
    ...
    call MDF_Inq_DimID( hid, 'lon', dimid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Inquire_Dimension( hid, dimid, status, length=nlon )
    IF_NOTOK_RETURN(status=1)
    ! storage:
    allocate( lons(nlon) )
    ! read:
    call MDF_Inq_VarID( hid, 'lon', varid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Get_Var( hid, varid, lons, status )
    IF_NOTOK_RETURN(status=1)
    
    ! number of latitudes:
    call MDF_Inq_DimID( hid, 'lat', dimid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Inquire_Dimension( hid, dimid, status, length=nlat )
    IF_NOTOK_RETURN(status=1)
    ! storage:
    allocate( lats(nlat) )
    ! read:
    call MDF_Inq_VarID( hid, 'lat', varid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Get_Var( hid, varid, lats, status )
    IF_NOTOK_RETURN(status=1)
    
    ! grid definition:
    call Init( lli_in, lons(1), lons(2)-lons(1), nlon, &
                       lats(1), lats(2)-lats(1), nlat, status )
    IF_NOTOK_RETURN(status=1)
    
    ! storage:
    allocate( swg_in(nlon,nlat) )
    ! read:
    call MDF_Inq_VarID( hid, 'swg', varid, status )
    IF_NOTOK_RETURN(status=1)
    call MDF_Get_Var( hid, varid, swg_in, status )
    IF_NOTOK_RETURN(status=1)
    ! get units ..
    call MDF_Get_Att( hid, varid, 'units', units_in, status )
    IF_NOTOK_RETURN(status=1)
    ! check ...
    if ( trim(units_in) /= trim(units) ) then
      write (gol,'("do not know how to convert swg units `",a,"` to `",a,"`")') &
    ...
    ! regrid:
    call FillGrid_AreaAverage( lli, swg, lli_in, swg_in, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine roadresp_dust_em( emis, t, status )
    ...
    use LE_Emis_Country, only : country_map_n, country_map_code, country_map_frac
#ifdef with_labeling
    ...
    do i=1,nx
      do j=1,ny
        ...
        !date and location definitions
        lon = runF%dlon*i+runF%westb
        lat = runF%dlat*j+runF%southb

        !MSP quick fix: get country code from country_map_code
        country = 'xxx'
        do k = 1, country_map_n
           if (country_map_frac(i,j,k) .gt. 0.2 ) country = country_map_code(k)
        enddo
        !print *, country
        if(country.eq.'xxx') country = 'GBR'

        call local_time2(country, t%year, t%month, t%day, t%hour, &
                           yyh, mmh, ddh, hhh, weekday, status)
        IF_NOTOK_RETURN(status=1)                           
        !print*,'local_time2, hh, hhh', country(i,j), hh, hhh
