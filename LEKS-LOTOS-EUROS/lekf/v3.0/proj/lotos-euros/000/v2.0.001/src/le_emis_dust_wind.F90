...
contains
  ...
  subroutine dust_wind_potential_sources( rcF, status )
    ...
    if ( trim(fname) /= '' ) then 
        ...
        ! storage:
        allocate( pot_sourceX(nlon,nlat) )
        ...
        status = NF90_Inq_VarID( ncid, trim('level'), varid )
        IF_NF90_NOTOK_RETURN(status=1)
        status = NF90_Get_Var( ncid, varid, pot_sourceX )
        IF_NF90_NOTOK_RETURN(status=1)
        ...
        allocate( pot_source(nx,ny) )
        call FillGrid_AreaAverage( lli, pot_source, lliX, pot_sourceX, status )
        IF_NF90_NOTOK_RETURN(status=1)
        
        ! clear
        deallocate( pot_sourceX )
  ...
  subroutine LE_Emis_Dust_Wind_Init( rcF, status )
    ...
    IF_NOTOK_RETURN(status=1)
    ! output file:
    write (filename,'(3a)') trim(outdir), pathsep, 'dust-threshold-factor-clay.csv'
    ! file unit:
  ...
  subroutine LE_Emis_Dust_Wind_Add( emis, t, status )
    ...
    use LE_LandUse_Data, only : z0dust_emis_lu
    use LE_LandUse_Data, only : z0m_depac, ilu_desert, ilu_arable, ilu_water_sea
    use LE_LandUse_Data, only : z0m_lu
      ...
      do j = 1, ny
        ...
        do i_erodible = 1, n_erodible
          ...
          do i_soiltexture = 1, n_soiltexture
            ...
            em_mode = em_mode + em_frac * frac * area ! kg/s
