...
contains
  ...
  subroutine LE_Bound_Clim_Logan_Init( path, status )
    ...
    if ( i_o3 > 0 ) then
      ...
      ! check ...
      if ( (runF%westb  <  west_logan-          0.5 *dx_logan) .or. &
           (runF%eastb  >  west_logan+(nx_logan-0.5)*dx_logan) .or. &
           (runF%southb < south_logan-          0.5 *dy_logan) .or. &
           (runF%northb > south_logan+(ny_logan-0.5)*dy_logan) ) then
        ...
        write (gol,'("  model w,e,s,n       : ",4f8.2)') runF%westb, runF%eastb, runF%southb, runF%northb; call goErr
        TRACEBACK; stop
      end if
  ...
  subroutine get_logan( iyear, imonth, spec_filled, status )
    ...
    use dims, only : nspec
    use indices
    use dims, only : bc_west, bc_east, bc_south, bc_north, caloft, runF
   ...
   if((iyear>=MH_YEAR1).and.(iyear<=MH_YEAR2))then
     macehead_O3=macehead_year(:,iyear)
      ...
      do k=1,nz
         ...
         enddo
         ix = int( (runF%westb-west_logan)/dx_logan) + 1
         iy = int( (runF%southb+(j-0.5)*runF%dlat-south_logan)/dy_logan) + 1
         ...
         do kk = 1, nz_logan
            if (press < levels_logan(kk)) ind = kk
         enddo
         ix = int( (runF%westb+nx*runF%dlon-west_logan)/dx_logan) + 1
         iy = int( (runF%southb+(j-0.5)*runF%dlat-south_logan)/dy_logan) + 1
      ...
      do k=1,nz
         ...
         do kk = 1, nz_logan
            if (press < levels_logan(kk)) ind = kk
         enddo
         ix = int( (runF%westb+(i-0.5)*runF%dlon-west_logan)/dx_logan) + 1
         iy = int( (runF%southb-south_logan)/dy_logan) + 1
         ...
         enddo
         ix = int( (runF%westb+(i-0.5)*runF%dlon-west_logan)/dx_logan) + 1
         iy = int( (runF%southb+ny*runF%dlat-south_logan)/dy_logan) + 1
