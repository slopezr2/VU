          ...
          if ( (dat%list_t(ipl) > t1) .and. (dat%list_t(ipl) <= t2) ) then
            ...
            do j = 1, tmodis%ny
              do i = 1, tmodis%nx
                ...
                ! check location:
                call In_Domain( lli, real(tmodis%Longitude(i,j)), real(tmodis%Latitude(i,j)), indomain, status )
                IF_NOTOK_RETURN(status=1)
                ...
                call GetLocation( lli, dat%pix_Longitude(ipix), dat%pix_Latitude(ipix), &
