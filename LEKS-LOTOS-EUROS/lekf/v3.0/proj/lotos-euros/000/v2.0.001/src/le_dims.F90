...
module dims
  ...
  type runspec
    ! the actual grid sizes
    real            ::  dlon, dlat
    ! the south west, north and east boundary
    real            ::  westb, southb, northb, eastb
    ! the grid sizes in km
    real            ::  dy
    real, pointer   ::  dx(:)
    !
  ...
  ! stability fields:
  real, allocatable, dimension(:,:) :: ustar ! deduced meteo field
  real, allocatable, dimension(:,:) :: monin ! deduced meteo field
  ...
  real,allocatable, dimension(:,:,:) :: bc_west,  bc_east, bc_north, bc_south, caloft
...
!    allocate(rb(nx,ny,nspec) )
    ...
    !allocate(cvf(nx,ny,nz) ) 
    allocate(ustar(nx,ny) ) 
    allocate(monin(nx,ny) ) 
...
#endif
    allocate(bc_west(ny,nz,nspec) )
    allocate(bc_east(ny,nz,nspec) ) 
    allocate(bc_north(nx,nz,nspec) ) 
    allocate(bc_south(nx,nz,nspec) ) 
    allocate(caloft(nx,ny,nspec) ) 
