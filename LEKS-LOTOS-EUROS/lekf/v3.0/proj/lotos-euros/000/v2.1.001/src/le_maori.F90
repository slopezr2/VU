        ...
        select case ( ptype )
          ...
          case ( MAORI_INT )
            ...
            select case ( trim(name_unit) )
              case ( 'grid_nlon [1]' ) ; ival = lli%nlon  ! grid size
              case ( 'grid_nlat [1]' ) ; ival = lli%nlat  ! grid size
          ...
          case ( MAORI_REAL )
            ! extract value from model data in requested unit:
            select case ( trim(name_unit) )
              case ( 'bound_west [degree_east]'   ) ; rval = lli%blon_deg(0)
              case ( 'bound_east [degree_east]'   ) ; rval = lli%blon_deg(lli%nlon)
              case ( 'bound_south [degree_north]' ) ; rval = lli%blat_deg(0)
              case ( 'bound_north [degree_north]' ) ; rval = lli%blat_deg(lli%nlat)
              case default
                write (gol,'("unsupported real parameter name [unit] : ",a)') trim(name_unit); call goErr
                TRACEBACK; status=1; return
            end select
      ...
      select case ( stype )
        ...
        ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          do iloc = 1, nloc
            ...
            ! get cell indices:
            call GetLocation( lli, lon, lat, ilon, ilat, status )
            if (status/=0) then
