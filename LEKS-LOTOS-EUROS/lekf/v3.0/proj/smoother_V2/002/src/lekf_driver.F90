...
contains
  ...
  subroutine LEKF_TimeStep_Run( t, dt, x, is_model, status, xb )
    ...
    use LEKF_State , only : TState
    use LEKF_State , only : bud0
    !Santiago SO2_rate perturb
    use LE_Chem     ,only : rk_pertur
    use LE_Chem     ,only : SO2_rate_pertur
    ! --- in/out ----------------------------
    ...
    !Santiago
    if (.not. allocated(rk_pertur)) allocate( rk_pertur(nx,ny) )
    
    !For default, start SO2_rate_pertur in false
    SO2_rate_pertur = .false.
    ...
    do inoise = 1, nnoise
      ...
      select case ( trim(noise_name(inoise)) )
        ...
        ! surface emissions and adjacent boundary Primary Particulate Matter: Santiago
        case ( 'PPM_emis' )
          ...
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_ppm_f) = emis_a(:,:,iz,i_ppm_f) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_ppm_c ) = emis_a(:,:,iz,i_ppm_c ) * x%dc(:,:,inoise,1) * dfc(inoise)
            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_ppm_f) = bc_west (:,iz,i_ppm_f) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_ppm_c ) = bc_west (:,iz,i_ppm_c ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_ppm_f) = bc_east (:,iz,i_ppm_f) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_ppm_c ) = bc_east (:,iz,i_ppm_c ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_ppm_f) = bc_south(:,iz,i_ppm_f) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_ppm_c ) = bc_south(:,iz,i_ppm_c ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_ppm_f) = bc_north(:,iz,i_ppm_f) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_ppm_c ) = bc_north(:,iz,i_ppm_c ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels

            
        ! surface emissions and adjacent boundary Black (Elementary Carbon): Santiago
        case ( 'BC_emis' )

          ! loop over levels:
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_ec_c) = emis_a(:,:,iz,i_ec_c) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_ec_f ) = emis_a(:,:,iz,i_ec_f ) * x%dc(:,:,inoise,1) * dfc(inoise)
            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_ec_c) = bc_west (:,iz,i_ec_c) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_ec_f ) = bc_west (:,iz,i_ec_f ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_ec_c) = bc_east (:,iz,i_ec_c) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_ec_f ) = bc_east (:,iz,i_ec_f ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_ec_c) = bc_south(:,iz,i_ec_c) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_ec_f ) = bc_south(:,iz,i_ec_f ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_ec_c) = bc_north(:,iz,i_ec_c) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_ec_f ) = bc_north(:,iz,i_ec_f ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels

        ! surface emissions and adjacent boundary Dust: Santiago
        case ( 'dust_emis' )
          ! loop over levels:
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_dust_c1) = emis_a(:,:,iz,i_dust_c1) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_c2) = emis_a(:,:,iz,i_dust_c2) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_c3)  = emis_a(:,:,iz,i_dust_c3)  * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_f1 ) = emis_a(:,:,iz,i_dust_f1 ) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_dust_f2 ) = emis_a(:,:,iz,i_dust_f2 ) * x%dc(:,:,inoise,1) * dfc(inoise)

            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_dust_c1) = bc_west (:,iz,i_dust_c1) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_c2) = bc_west (:,iz,i_dust_c2) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_c3)  = bc_west (:,iz,i_dust_c3)  * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_f1 ) = bc_west (:,iz,i_dust_f1 ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_dust_f2) = bc_west (:,iz,i_dust_f2) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_c1) = bc_east (:,iz,i_dust_c1) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_c2) = bc_east (:,iz,i_dust_c2) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_c3)  = bc_east (:,iz,i_dust_c3)  * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_f1 ) = bc_east (:,iz,i_dust_f1 ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_dust_f2 ) = bc_east (:,iz,i_dust_f2 ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_c1) = bc_south(:,iz,i_dust_c1) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_c2) = bc_south(:,iz,i_dust_c2) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_c3)  = bc_south(:,iz,i_dust_c3)  * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_f1 ) = bc_south(:,iz,i_dust_f1 ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_dust_f2 ) = bc_south(:,iz,i_dust_f2 ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_c1) = bc_north(:,iz,i_dust_c1) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_c2) = bc_north(:,iz,i_dust_c2) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_c3)  = bc_north(:,iz,i_dust_c3)  * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_f1 ) = bc_north(:,iz,i_dust_f1 ) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_dust_f2 ) = bc_north(:,iz,i_dust_f2 ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels

        ! surface emissions and adjacent boundary Primary Organic Matter: Santiago
         case ( 'POM_emis' )

          ! loop over levels:
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_pom_f) = emis_a(:,:,iz,i_pom_f) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_pom_c ) = emis_a(:,:,iz,i_pom_c ) * x%dc(:,:,inoise,1) * dfc(inoise)
            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_pom_f) = bc_west (:,iz,i_pom_f) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_pom_c ) = bc_west (:,iz,i_pom_c ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_pom_f) = bc_east (:,iz,i_pom_f) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_pom_c ) = bc_east (:,iz,i_pom_c ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_pom_f) = bc_south(:,iz,i_pom_f) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_pom_c ) = bc_south(:,iz,i_pom_c ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_pom_f) = bc_north(:,iz,i_pom_f) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_pom_c ) = bc_north(:,iz,i_pom_c ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels


         ! surface emissions and adjacent boundary Sodium: Santiago
        case ( 'Na_emis' )
          ! loop over levels:
          do iz = 1, nz
            ! scale emissions:
            emis_a(:,:,iz,i_na_c1) = emis_a(:,:,iz,i_na_c1) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_c2) = emis_a(:,:,iz,i_na_c2) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_c3) = emis_a(:,:,iz,i_na_c3) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_f1 ) = emis_a(:,:,iz,i_na_f1 ) * x%dc(:,:,inoise,1) * dfc(inoise)
            emis_a(:,:,iz,i_na_f2 ) = emis_a(:,:,iz,i_na_f2 ) * x%dc(:,:,inoise,1) * dfc(inoise)

            ! scale lowest boundary layer:
            if ( iz == 1 ) then
              bc_west (:,iz,i_na_c1) = bc_west (:,iz,i_na_c1) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_c2) = bc_west (:,iz,i_na_c2) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_c3) = bc_west (:,iz,i_na_c3) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_f1 ) = bc_west (:,iz,i_na_f1 ) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_west (:,iz,i_na_f2) = bc_west (:,iz,i_na_f2) * x%dc( 1, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_c1) = bc_east (:,iz,i_na_c1) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_c2) = bc_east (:,iz,i_na_c2) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_c3) = bc_east (:,iz,i_na_c3) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_f1 ) = bc_east (:,iz,i_na_f1 ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_east (:,iz,i_na_f2 ) = bc_east (:,iz,i_na_f2 ) * x%dc(nx, :,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_c1) = bc_south(:,iz,i_na_c1) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_c2) = bc_south(:,iz,i_na_c2) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_c3) = bc_south(:,iz,i_na_c3) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_f1 ) = bc_south(:,iz,i_na_f1 ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_south(:,iz,i_na_f2 ) = bc_south(:,iz,i_na_f2 ) * x%dc( :, 1,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_c1) = bc_north(:,iz,i_na_c1) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_c2) = bc_north(:,iz,i_na_c2) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_c3) = bc_north(:,iz,i_na_c3) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_f1 ) = bc_north(:,iz,i_na_f1 ) * x%dc( :,ny,inoise,1) * dfc(inoise)
              bc_north(:,iz,i_na_f2 ) = bc_north(:,iz,i_na_f2 ) * x%dc( :,ny,inoise,1) * dfc(inoise)
            end if
          end do  ! levels


        case('aerosol_emis')
            aerosoles=(/i_ppm_f,i_ppm_c,i_ec_c,i_ec_f,i_dust_c1,i_dust_c2,i_dust_c3,i_dust_f1,i_dust_f2,i_pom_c,i_pom_f,i_na_c1,i_na_c2,i_na_c3,i_na_f1,i_na_f2 /)
            
            do iaero=1,16 !Do of aerosol species
                do iz = 1, nz
                ! scale emissions:
                    emis_a(:,:,iz,aerosoles(iaero)) = emis_a(:,:,iz,aerosoles(iaero)) * x%dc(:,:,inoise,1) * dfc(inoise)      
                    ! scale lowest boundary layer:
                    if ( iz == 1 ) then
                        bc_west (:,iz,aerosoles(iaero)) = bc_west (:,iz,aerosoles(iaero)) * x%dc( 1, :,inoise,1) * dfc(inoise)
                        bc_east (:,iz,aerosoles(iaero)) = bc_east (:,iz,aerosoles(iaero)) * x%dc(nx, :,inoise,1) * dfc(inoise)
                        bc_south(:,iz,aerosoles(iaero) ) = bc_south(:,iz,aerosoles(iaero)) * x%dc( :, 1,inoise,1) * dfc(inoise)
                        bc_north(:,iz,aerosoles(iaero)) = bc_north(:,iz,aerosoles(iaero)) * x%dc( :,ny,inoise,1) * dfc(inoise)
                    end if
                end do  ! levels
            end do !aerosoles
        ...
        !Santiago SO2-->SO4a_f +HO rate noise
        case ( 'SOx_rate' )
            ! save the values from x%dc in the public variable rk_pertur:
            rk_pertur = x%dc(:,:,inoise,1) * dfc(inoise)
            ! put true the flaq to indicate that SO2 rate is perturbed
            SO2_rate_pertur = .true.
            write(*,*)'Santiago LEKF_Driver======'
            write(*,*)SO2_rate_pertur
    ...
    if ( .not. is_model ) then
      ...
      do inoise = 1, nnoise
        ...
        select case ( trim(noise_name(inoise)) )
          ...
          ! already done ...
          case ( 'None' )
          case ( 'NOx_emis', 'VOC_emis', 'SOx_emis', 'NH3_emis', 'PM25_BC_emis' ,'HCHO_emis' , 'PPM_emis', 'BC_emis', 'POM_emis', 'dust_emis', 'Na_emis', 'aerosol_emis' )
          case ( 'o3_top', 'o3_vd' )
          case ( 'dust_bc' )
          case ( 'SOx_rate' )
