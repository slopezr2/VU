...
contains
  ...
  subroutine LEKF_Output_DC_State_Init( self, rcF, rckey, name, status )
    ...
    ! init common stuff:
!     call Init( self%com, rcF, rckey, status ) Santiago
    call self%com%Init( rcF, rckey, status )
...
!     call Done( self%com, status ) Santiago
    call self%com%Done( status )
...
!     use LE_Output_Common, only : PutOut_GlobalAttributes
    ...
    if ( (t < self%tr(1)) .or. (self%tr(2) < t) ) then
      ...
      if ( goc%root ) then
        ...
        ! write global attributes: Santiago
!         call PutOut_GlobalAttributes( self%com, self%ncid, status )
        call self%com%PutOut_GlobalAttributes( self%ncid, status )
