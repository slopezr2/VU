...
class LE_model:
	...
	def __init__(self):
		...
		self.start_date = ''
		self.end_date = ''
		self.compile_command = './base/005/bin/setup-lekf proj/smoother/001/rc/lekf.rc'
		self.run_command = 'srun  --kill-on-bad-exit = 1 --output = le.run.out.%t  --error = le.run.err.%t  ./lekf.x lekf.rc'
		self.run_path = '/scratch-shared/slr/projects/Smoother/'
		self.lekf_rc_path = '../proj/smoother/001/rc/'
	...
	def write_ID(self):
		...
		file = self.lekf_rc_path+'lekf.rc'
		status = 1
		try:
	...
	def write_date(self):
		...
		"""
		file = self.lekf_rc_path+'lotos-euros-v2.2.001.rc'		
	...
	def write_N_ensembles(self):
		"""
		Write the number of ensembles in the lekf.rc file.
