...
Tb = 356		 #Background step. The total days of simulation.
...
# 		LE.run()
		...
		for ens in range(1,LE.Nens+1):
			v1 = 1.5+np.random.randn(1)*0.1
			v2 = 4+np.random.randn(1)*0.1
			v3 = 2.5+np.random.randn(1)*0.1
			v4 = 3.5+np.random.randn(1)*0.1
			aux_dc[:] = max(1,v1)
			aux_dc[:,:,:,80:,:] = max(1,v2)
