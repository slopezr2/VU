		...
		for ens in range(1,LE.Nens+1):
			...
			for idate,date in enumerate(dates_cycle_new_cycle):
				...
				if i_day_LETKF_bef!=i_day_LETKF: #Actualize just for a new Ts day
					aux_dc[:,0,:,:,:] = LETKF.create_rand_ensemble_dc(new_mean=1,new_std=1,r=radius+1,auto_shape=False,nnoise=dc_dim[2],nlat=dc_dim[3],nlon=dc_dim[4]) #The same value for all the hours of the day
